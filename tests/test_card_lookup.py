...
def test_a_series_does_not_answer_to_a_movie_key_of_another_year() -> None:
    ...
    Разошлись оба - это уже другая картина: замер на живом приёмнике 07-09-2026 отдал по
...
def test_a_tile_key_answers_when_only_the_year_drifted() -> None:
    ...
    Замер на живом приёмнике 07-09-2026: плитка «Укрытие» несёт ``tv:укрытие:2026`` (в окне
...
def test_a_tile_key_answers_through_a_typo_when_kind_and_year_both_agree() -> None:
    """Лента и выдача пишут одно имя по-разному: замер на живом приёмнике 11-09-2026 - плитка
...
def test_a_picture_answers_to_the_key_made_of_the_name_the_feed_gave_it() -> None:
    ...
    Замер на живом приёмнике 07-09-2026: круг по запросу «Better Days» отдаёт картину
