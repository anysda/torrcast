"""Счастливый путь §2 SPEC-v2: фильм и озвучка, больше ничего.

Проверяется дословное требование владельца: «не хочу видеть какие там файлы, хочу
...
#: Настоящее ожидание картинки: фикстура стенда подменяет его заглушкой, а один тест
...
def _stand(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Стенд без Prowlarr, без TorrServer, без systemd — но с полным путём выбора."""
    monkeypatch.setenv("TORRCAST_STATE", str(tmp_path / "state.json"))
    monkeypatch.setenv("TORRCAST_CONFIG", str(tmp_path / "config.json"))
    save_config(Config(tv="192.168.100.102", prowlarr_apikey="ключ", hls_dir=str(tmp_path / "hls")))
    monkeypatch.setattr(cli, "Prowlarr", _FakeProwlarr)
    monkeypatch.setattr(cli, "TorrServer", _FakeTorrServer)
    monkeypatch.setattr(cli, "probe", lambda url, timeout=90.0: Media(5978.0, TRACKS, "h264", 1080))
...
def test_the_happy_path_asks_about_the_film_and_nothing_else(
    ...
    # И ни одного значка из запрещённых §3.
...
def test_the_film_with_a_number_in_the_title_is_a_film(
    ...
    """Дефект №3 владельца: «Моана 2» — фильм, и строк про серии в её выводе нет."""
...
def test_releases_prints_the_old_table_and_exits(capsys: pytest.CaptureFixture[str]) -> None:
    """`cast releases <запрос>` — та самая таблица, но по явной просьбе (§2 SPEC-v2)."""
...
def test_the_start_time_means_a_picture_on_the_screen(
    ...
    """«Старт NN с» обязан означать картинку, а не «упаковка пошла» (§4 SPEC-v2).
