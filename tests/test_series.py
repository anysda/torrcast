"""Серии = файлы раздачи (§2.4): маппинг файлов в ``sNeM`` и отбор релизов по сезону.

Имена файлов взяты из живой выдачи dev-Prowlarr и метаданных dev-TorrServer
(разведка этапа 4, docs/stage4.md): «Киберпанк: Бегущие по краю», «Во все тяжкие»,
«Друзья», «Клиника», «Наруто». Ловушки тут не выдуманные — каждая встретилась.
"""
...
def test_season_pack_maps_every_file_to_its_own_season() -> None:
    """Пак сезонов: серии нескольких сезонов, каждый файл — на свой ``sNeM`` (§2.4).
...
def test_release_name_tells_which_seasons_it_covers() -> None:
    """Отбор релизов под нужный сезон (§2.4): пак `[S01-06]` покрывает шестой сезон,
...
def test_the_episode_is_cut_off_the_query(
    ...
    """§2.4: указание серии отделяется от запроса — в Prowlarr уедет «киберпанк», а не
    «киберпанк 2x5». Номер франшизы («матрица 2») серией не считается (§2.2).
    """
