...
class _Blinking:
    ...
    приёмник, бросивший показ, LOAD какое-то время не берёт (``refuses``), поэтому
    попытка, выстрелившая сразу, сгорает впустую. ⚠️ Причина тут НЕ обида на 404: её нет
    вовсе, ноль секунд (замер 09-08-2026 на живом Samsung Q70D, опровергнут трижды).
    Заглушка моделирует занятый экран - приложение приёмника висит на нём 301 с после
    смерти медиасессии.
    """
...
def test_a_try_is_not_burnt_on_a_receiver_that_only_just_dropped_the_show(
    ...
    LOAD не берёт вовсе - не из-за обиды на 404 (её нет, замер даёт ноль секунд), а
    потому что экран ещё занят его же приложением, - и попытка сгорала впустую: одна из
    трёх на весь показ.
