...
def test_the_live_series_climbs_over_the_silent_one_seed_packs() -> None:
    """Порядок очереди на живой выдаче «Чёрных парусов»: 61 сид поднимается с 5-го на 2-е.
    ...
    Место третье стало вторым после TC-826: ступень звука встала выше ступени старья, и
    названный вслух MVO обошёл ещё и молчаливый 720p-пак BaibaKo, который держался на
    том, что моложе. Сама суть теста от этого не поехала - живой сериал по-прежнему
    поднимается над односидными паками, только теперь на место выше.
    """
    tv = RUNTIME_GUESS["tv"]
    releases = [
        _series_release(
    ...
    assert order.index(int(32.53 * 1e9)) == 1, "живой сериал второй, а не пятый"
    assert order.index(int(27.24 * 1e9)) > 1, "молчаливые односидные паки ушли ниже него"
    assert order.index(int(10.24 * 1e9)) > 1
    assert order.index(int(17.32 * 1e9)) > 1, "и молчаливый 720p-пак: возраст звука не перевесил"
    assert order[0] == int(114.21 * 1e9), "честный 720p с верха не сходит"


def test_a_named_russian_track_outranks_the_silent_one_that_only_looks_newer() -> None:
    """🟠 TC-826: ступень звука стоит ВЫШЕ ступени старья - живая выдача «Безумного Макса».

    Порядок ступеней - решение владельца: возраст релиза говорит про КАРТИНКУ, а звук
    по лестнице цели сильнее качества картинки. До перестановки первым в очереди стоял
    молчащий про звук ``BDRip 1080p`` с двумя сидами, а ``MVO + Original Eng`` с 29
    сидами - вторым; молчун стоит лишних чтений паспорта, а на дохлом рое итогом
    становится честный отказ вместо показа.

    Имена, размеры и сиды взяты из офлайн-переигровки корпуса
    (``scripts/poolreplay.py``, 100 запросов): там же посчитана и цена перестановки -
    первый релиз сменился у 12 картин из 441, у всех двенадцати звук стал назван вслух,
    и ни у одной не сменилась сама картина.
    """
    releases = [
        _series_release(
            "Безумный Макс / Mad Max [1979, Австралия, боевик, BDRip] MVO + Original Eng (USA)",
            1.58,
            29,
        ),
        _series_release("Безумный Макс / Mad Max (1979) BDRip 1080p", 9.43, 2),
        _series_release(
            "Безумный Макс / Mad Max [1979, Австралия, боевик, триллер, приключения, BDRip] "
            "AVO + AVO",
            1.80,
            5,
        ),
    ...
    order = [r.size for r in rank_releases(releases, RUNTIME, 16.0)]

    assert order[0] == int(1.58 * 1e9), "названный вслух MVO берётся первым"
    assert order.index(int(9.43 * 1e9)) == 2, "молчащий про звук 1080p ушёл под обе озвучки"
