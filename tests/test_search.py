...
def test_пустой_поиск_при_бане_кворумного_называет_урезанный_каталог() -> None:
    ...
    ловит :func:`~torrcast.cli._ask`, а вместе с ним пропадали и добор второй строкой, и
    уже собранный пул. Замер офлайн (10 запросов, молчит один Knaben, остальные живы):
...
def test_молчание_кворумного_названо_в_урезанном_каталоге() -> None:
    ...
    with pytest.raises(NotFoundError, match="каталог сейчас урезан - молчит Knaben") as caught:
        client.search("матрица")
    print(str(caught.value))
...
def test_молчание_круга_не_отменяет_поиск_в_котором_нам_уже_отвечали() -> None:
    ...
    инфры, а отказ инфры не ловит :func:`~torrcast.cli._ask`: вместе с добором пропадал
    и уже собранный пул. Замер офлайн: так пропадало готовое меню у 4 запросов из 10.
    ...
    client._session.mute_all = True  # type: ignore[union-attr]
