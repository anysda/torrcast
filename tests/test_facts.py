...
def test_an_answered_emptiness_is_told_apart_from_a_reference_that_never_came() -> None:
    ...
    ``X-Torrcast-Partial`` картину без статьи навсегда: замер 10-09-2026 на живом
    приёмнике - страница ходила за `tv:пассажиры-2:2022` шесть раз и умолкала только своим
...
def test_menu_prints_the_old_line_when_there_is_no_help() -> None:
    ...
    assert menu_blocks(plans, None, width=80) == [
...
def test_menu_puts_rating_and_time_in_the_head_and_the_plot_below() -> None:
    ...
    printed = [line for block in menu_blocks(plans, facts, width=80) for line in block]
...
def test_the_description_wraps_by_words_under_the_terminal() -> None:
    ...
    printed = [line for block in menu_blocks(_moana_franchise(), facts, width=60) for line in block]
