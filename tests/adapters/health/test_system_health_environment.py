...
def test_the_environment_answers_every_question_of_the_port() -> None:
    """Порт спрашивает про машину, службы и темп ffmpeg разом, а спрашиваются они порознь."""
...
def test_the_environment_adds_nothing_of_its_own() -> None:
    """Своих методов у сборки нет: она только сводит три пробы."""
    assert not {name for name in vars(SystemHealthEnvironment) if not name.startswith("_")}
    assert SystemHealthEnvironment.__mro__[1:4] == (MachineProbe, ServiceProbe, FfmpegPaceProbe)
