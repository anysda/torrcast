...
def test_the_head_is_the_very_beginning_of_the_file() -> None:
    ...
    assert pull_head("http://торрент/поток", warm=note) == HEAD_WARM
    assert asked == [("http://торрент/поток", 0, HEAD_WARM, None)]


def test_the_size_and_the_liveness_are_passed_on() -> None:
    ...
    pull_head("http://торрент/поток", 1 << 20, alive, warm=note)
