...
def test_a_keyless_copy_after_a_recode_on_a_flat_grid_is_shrunk_before_the_seam(
    tmp_path: Path,
) -> None:
    ...
    _lay_out(
        ...
        after_recode=lambda slot: slot == 0,
...
def test_a_keyless_copy_after_a_copy_on_a_flat_grid_stays_a_copy(tmp_path: Path) -> None:
    """Соседняя копия уже несёт PPS того же потока, ужатие только замедлит показ."""
    told: list[tuple[int, str]] = []
    spare = tmp_path / "recode"
    spare.mkdir()
    run = packer(tmp_path, spare=spare, told=lambda slot, how: told.append((slot, how)))
    ...
    _lay_out(
        ...
        _always,
        keyless=lambda piece: piece == run.run / "v0.ts",
        after_recode=lambda slot: False,
        starts_of=_on_place,
    )
