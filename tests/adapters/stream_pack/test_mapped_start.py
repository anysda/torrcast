...
def test_an_editless_mp4_is_sought_by_decode_time() -> None:
    """У mp4 без списка правок индекс ffmpeg стоит на dts: пыльная граница - сам кадр.

    Замер TC-699 на живом YIFY-релизе 2014 (start_time 0.083, ctts на всей дорожке):
    ``-ss`` в полосе ctts ПЕРЕД опорным кадром садится на САМ кадр, поэтому бисект идёт
    по исковому времени (``via`` = dts), а не по метке показа. Без этого заход по карте
    на границу 9.259 обещал бы посадку 2.085 - промах на опорный кадр, и сверка с
    пробным прогоном не сошлась бы никогда.
    """
    keys = FilmKeys(
        60.0,
        [0.0834, 2.0854, 9.25925, 17.3507],
        [0, 1, 2, 3],
        "mp4",
        via=(0.0, 2.002, 9.17585, 17.2673),
    )
    assert mapped_start(keys, 9.25925) == 9.25925, "пыльная граница - сам кадр, а не прежний"
    assert mapped_start(keys, 13.305) == 9.25925, "внутри GOP - последний кадр перед местом"
