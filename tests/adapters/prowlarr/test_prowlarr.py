...
def test_slow_extra_indexer_does_not_hold_the_ready_catalog() -> None:
    ...
    assert elapsed < 0.5
    # Кто ещё в пути - признак неполноты выдачи, и спрашивает его лента:
    # поле `late` следа круга.
...
def test_поздний_ответ_живёт_дольше_бюджета_круга() -> None:
    ...
    # Срок долива - цена независания, а не предмет замера: предмет здесь в том, что
    # поздний ответ приезжает ЦЕЛИКОМ, а не в том, за сколько. Под нагрузкой прежние
    # 0.2 с при задержке 0.06 с не переживали планировщика (1 красный прогон из 54), и
    # соседние пробы долива берут его с тем же запасом.
    late = client.late(wait=5.0)
    assert [r.indexer for r in late] == ["idx.1", "idx.1"]
    assert _swarm_of(client).budget["1"] > 0.06


def test_потолок_первого_круга_отпускает_меню_не_дожидаясь_кворумного() -> None:
    """🔴 TC-1046: первый круг уходит по СВОЕМУ потолку, а не по бюджету самого
    медленного опорного.

    Личный бюджет Knaben здесь настоящий (20 с, TC-226) - режет его именно потолок круга
    (:attr:`Prowlarr.first_cap`). Отличие от TC-454 в этом: там круг ждал столько, сколько
    ему назвали про ОДИН источник, тут - столько, сколько назвали про весь круг.

    Вторая половина обещания важнее первой: отставший не выброшен. Его запрос живёт свой
    полный срок, и те же строки приезжают доливом.
    """
    client = _swarm(slack=0.0, rows=2, delay={1: 0.6})
    client.first_cap = 0.1  # потолок круга ниже ответа Knaben, но бюджет его нетронут

    results = client.search("матрица")

    assert [r.indexer for r in results] == ["idx.2", "idx.2", "idx.3", "idx.3"], (
        "меню собрано без опоздавшего, а не отложено до его ответа"
    )
    late = client.late(wait=2.0)
    assert [r.indexer for r in late] == ["idx.1", "idx.1"], "строки опоздавшего не потеряны"
    assert _swarm_of(client).budget["1"] >= LATE_TIMEOUT, (
        "потолок круга режет ожидание, а не срок самого запроса"
    )
