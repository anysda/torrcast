...
def test_молчание_части_пакетов_не_читается_как_статьи_нет_про_её_картины() -> None:
    ...
    ⚠️ Меню тут разноимённое нарочно. Границу пакета двигает ЛЮБОЕ прибавление в наборе
    уточнений (TC-844), и на трёх коротких именах она резала волну ровно посередине
    очереди каждой картины: за двадцатое имя уезжали все три, промолчавшими выходили тоже
    все, и сторож переставал различать спрошенное от неспрошенного. Короткое имя целиком
    влезает в первый пакет, а длинное с подзаголовком даёт вдвое больше кандидатов и
    хвостом уходит во второй - и запас тут в целый пакет, а не в одно имя.
    """

    def deaf(host: str, path: str, params: dict[str, str]) -> Any:
        if "властелин колец: братство кольца" in params["titles"].split("|"):
    ...
    client = FakeJsonClient(deaf)
    about, _entities, answered = _extracts(client, wanted, 1.0)

    assert len(client.calls) > 1, "стенд не состоялся: пакет один, и молчать в нём нечему"
    assert about[("Тачки", 2006)] == CARS, "ответившая часть разбирается как обычно"
    assert answered == {("Тачки", 2006)}, (
        "«Властелин колец» не спрошен до конца - «статьи нет» про него нечестно"
    )
