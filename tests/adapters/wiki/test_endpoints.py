...
def test_both_sources_are_named_by_host_and_path() -> None:
    ...
    assert endpoints._WIKI_HOST == "ru.wikipedia.org"
    assert endpoints._WIKI_PATH == "/w/api.php"
    assert endpoints._WIKIDATA_HOST == "query.wikidata.org"
    assert endpoints._WIKIDATA_PATH == "/sparql"
...
def test_sparql_is_asked_for_json_by_the_accept_header() -> None:
    ...
    assert endpoints._SPARQL_HEAD["Accept"] == "application/sparql-results+json"
