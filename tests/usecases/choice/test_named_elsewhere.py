"""Зеркало :mod:`torrcast.usecases.choice.named_elsewhere`: страж ухода с названного имени
с картины, чьё имя названо целиком.

На этом классе страж называет точно названную картину и ту, что встала дефолтом.
Класс узкий, и оба его условия проверены замером по корпусу-100: номерованная франшиза
...
def test_a_dead_exactly_named_picture_stops_the_default_and_names_the_reason() -> None:
    ...
    именем. Строка называет обе картины и причину; решение брать или спрашивать живёт
    выше.
    """
    bleach = [
