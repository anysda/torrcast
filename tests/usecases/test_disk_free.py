...
def test_the_free_space_comes_from_the_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(_state, "environment", FakeHealthEnvironment(free=7 * 1024**3))
...
def test_a_new_environment_is_answered_by_the_new_one(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(_state, "environment", FakeHealthEnvironment(free=1))
    first = disk_free("/tmp")
    monkeypatch.setattr(_state, "environment", FakeHealthEnvironment(free=2))
