...
def test_a_picture_without_a_bookmark_keeps_its_warm_under_the_menu() -> None:
    ...
    assert _plays_recorded(_state_with(None), plan(), Args(query=["кино"])) is False
...
def test_a_resumable_bookmark_answers_with_the_recorded_release() -> None:
    ...
    assert _plays_recorded(_state_with(entry()), plan(), Args(query=["кино"])) is True
...
def test_a_series_bookmark_does_not_answer_for_the_warm() -> None:
    ...
    assert _plays_recorded(_state_with(saved), plan(), Args(query=["кино"])) is False
...
def test_a_menu_picked_series_bookmark_answers_for_the_warm() -> None:
    ...
    assert _plays_recorded(_state_with(saved), own, Args(query=["кино"], menu=True)) is True
    named = Args(query=["кино", "s1e1"], menu=True)
    assert _plays_recorded(_state_with(saved), own, named) is False


def test_a_finished_bookmark_does_not_answer_for_the_warm() -> None:
    """Продолжать нечего - и сносить прогретое закладка не будет."""
    assert _plays_recorded(_state_with(entry(done=True)), plan(), Args(query=["кино"])) is False
...
def test_a_named_episode_keeps_the_warm_of_the_film() -> None:
    ...
    assert _plays_recorded(_state_with(entry()), plan(), Args(query=["кино", "s1e1"])) is False


def test_a_hand_named_release_keeps_the_warm() -> None:
    """``--release N`` играет выбранное руками: прогретое ещё пригодится."""
    assert _plays_recorded(_state_with(entry()), plan(), Args(query=["кино"], release=2)) is False
...
def test_from_start_answers_with_the_recorded_release_from_zero() -> None:
    ...
    assert (
        _plays_recorded(_state_with(entry()), plan(), Args(query=["кино"], from_start=True)) is True
    )
...
def test_a_buried_picture_keeps_its_warm_like_any_other() -> None:
    ...
    assert _plays_recorded(_state_with(entry()), plan(), args) is False
