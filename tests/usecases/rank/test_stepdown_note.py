...
def test_nothing_better_around_means_no_line() -> None:
    ...
    assert stepdown_note(_Plan([rel(name="полный", quality="1080p")]), 1, media(), [1]) == ""
...
def test_the_line_names_what_was_taken_and_what_stood_nearby() -> None:
    ...
    said = stepdown_note(_Plan([taken, near]), 1, media(height=720, width=1280), [1, 2], reached=1)
...
def test_the_taken_one_is_measured_by_its_passport_not_by_its_name() -> None:
    ...
    said = stepdown_note(_Plan([taken, near]), 1, media(height=574, width=1150), [1, 2], reached=1)
...
def test_a_dead_best_is_named_as_a_dead_swarm() -> None:
    ...
    said = stepdown_note(_Plan([taken, near]), 1, media(height=720, width=1280), [1, 2])
...
def test_a_rounding_difference_is_not_a_step() -> None:
    ...
    plan = _Plan([rel(name="взятый", quality="1080p"), rel(name="сосед", quality="1080p")])
