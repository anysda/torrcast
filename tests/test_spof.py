...
лёг - строка печатается прежней, бюджет не превышен. А вот
:func:`~torrcast.usecases.passport.Passport.of`
- это отдельный вход к той же справке, которым ходит ВТОРОЙ ЯЗЫК поиска
(:func:`torrcast.usecases.discover._second_language._second_language`). Ограждение у него то же
самое: справка не вправе ни ронять поиск, ни задерживать его сверх бюджета, а когда её нет - запрос
латиницей всё равно собирается (из выдачи, а в пределе транслитом).
