...
def test_menu_continuations_do_not_overrule_the_season_reading() -> None:
    ...
    зовущий (:func:`~torrcast.usecases.discover.season_reread._season_asked` отдаёт в разбор голое
    «токийский гуль»). Пополненная линейка приезжала в :func:`reads_season` с картиной, которой
    каталог назвал номер части, условие «ни одну не подписал номером» падало - и «токийский гуль 2»
    переставал просить второй СЕЗОН, отвечая полным метром. Ровно тот отказ, что закрыт в TC-363,
    только заходящий с другого конца.
    """
    pictures = cluster(
        [
            parse_release_name(name)
