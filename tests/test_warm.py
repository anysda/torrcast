...
# (:meth:`torrcast.usecases.feed_pack.feed.Feed.segment`), - и для приёмника это ОДНА лента. Если
# два производителя решают про кодирование по-разному, на стыке меняется SPS: другой профиль,
...
def test_warming_enters_the_run_exactly_where_the_live_packing_enters_it(
    ...
    (:func:`torrcast.adapters.stream_pack.pack_start.pack_start`). ffmpeg вставал раньше, резы
    захода уезжали на всю докатку, и первый кусок прогрева начинался на 1.7 с раньше своей границы:
    PCR и метки видео шли НАЗАД на стыке с живым куском. Там, где в сдвинутом окне не оказывалось
    опорного кадра, рез вставал верно и кусок совпадал с живым - поэтому сравниваются все куски
    захода, а не один.
    """
    grid = _offkey_grid()
...
# показа: цену соседства с прогревом (:data:`torrcast.adapters.recode.pace.COPY_TOLL`, 33 %)
