...
def test_a_record_from_before_the_frame_era_asks_the_passport_once(
    ...
    Без кадра перекод не знает, надо ли ужимать картинку под потолок приёмника, и 4К
    уезжает во весь свой кадр. Поэтому кадр добирается тем же одним ffprobe, что и
    глубина цвета, при первом же продолжении - и остаётся в записи.
    """
