"""Английский каталог кластера счёта серий: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.series.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("series.")]
    assert stray == []
    assert english()["series.none_found"] == "no episodes found"
