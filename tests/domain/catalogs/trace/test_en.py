"""Английский каталог кластера ленты меток: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.trace.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("trace.")]
    assert stray == []
    assert english()["trace.column_cost"] == "cost"
