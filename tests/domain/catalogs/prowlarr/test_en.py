"""Английский каталог кластера ``prowlarr``: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.prowlarr.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("prowlarr.")]
    assert stray == []
    assert english()["prowlarr.not_json"] == "Prowlarr returned something other than JSON"
