"""Английский каталог кластера поиска приёмников: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.chromecast_scan.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("chromecast_scan.")]
    assert stray == []
    assert english()["chromecast_scan.unnamed_device"] == "receiver"
