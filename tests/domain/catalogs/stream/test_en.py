"""Английский каталог кластера картинки и звука: он же умолчание, он же запасной.
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("stream.")]
    assert stray == []
    assert english()["stream.voice_swap"] == "voice {heard} instead of {studio}"
