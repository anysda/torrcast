"""Английский каталог кластера отбора: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.select.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("select.")]
    assert stray == []
    assert english()["select.phase_queue"] == "queue"
