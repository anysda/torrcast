"""Английский каталог кластера ``torrserver``: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.torrserver.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("torrserver.")]
    assert stray == []
    assert english()["torrserver.not_json"] == "TorrServer returned a non-JSON answer"
