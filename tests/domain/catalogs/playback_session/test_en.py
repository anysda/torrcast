"""Английский каталог кластера юнита показа: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.playback_session.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("playback_session.")]
    assert stray == []
    assert english()["playback_session.stream_address_unknown"] == "stream address is not known"
