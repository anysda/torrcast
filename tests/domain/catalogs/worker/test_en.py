"""Английский каталог кластера worker: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.worker.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("worker.")]
    assert stray == []
