"""Английский каталог кластера configure: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.configure.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("configure.")]
    assert stray == []
