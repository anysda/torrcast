"""Английский каталог кластера ``launchd``: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.launchd.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("launchd.")]
    assert stray == []
    assert english()["launchd.log_empty"] == "the log is empty"
