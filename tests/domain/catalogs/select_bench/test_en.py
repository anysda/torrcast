"""Английский каталог кластера стенда отбора: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.select_bench.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("select_bench.")]
    assert stray == []
    assert english()["select_bench.phase_done"] == "done"
