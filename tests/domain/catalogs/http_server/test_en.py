"""Английский каталог кластера ``http_server``: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.http_server.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("http_server.")]
    assert stray == []
    assert english()["http_server.address_unset"] == "(address not set)"
