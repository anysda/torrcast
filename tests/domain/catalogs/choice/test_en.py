"""Английский каталог кластера выбора: он же умолчание, он же запасной.
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("choice.")]
    assert stray == []
    assert english()["choice.question"] == "What are we watching?"
