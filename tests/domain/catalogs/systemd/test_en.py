"""Английский каталог кластера ``systemd``: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.systemd.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("systemd.")]
    assert stray == []
    assert english()["systemd.journal_empty"] == "the journal is empty"
