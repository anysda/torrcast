"""Английский каталог кластера пустого поиска: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.hunt.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("hunt.")]
    assert stray == []
    assert english()["hunt.refused"] == "a refusal at {names}"
