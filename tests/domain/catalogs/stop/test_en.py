"""Английский каталог кластера stop: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.stop.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("stop.")]
    assert stray == []
