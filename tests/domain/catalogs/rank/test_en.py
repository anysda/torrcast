"""Английский каталог кластера ранжирования: он же умолчание, он же запасной.
...
from torrcast.domain.catalogs.rank.en import en as english
...
def test_every_key_names_its_cluster() -> None:
    stray = [key for key in english() if not key.startswith("rank.")]
    assert stray == []
    assert english()["rank.lang_japanese"] == "Japanese"
