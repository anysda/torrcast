...
def test_the_pending_budget_covers_the_pieces_a_healthy_run_legitimately_holds() -> None:
    ...
    что придержаны под перекод (:data:`torrcast.adapters.recode.RUN_MAX`). На потолке веса
    это ``(RUN_MAX + 1)`` кусков, и порог берётся вдвое сверх - иначе здоровый показ на
