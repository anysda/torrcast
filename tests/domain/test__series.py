...
def test_the_episode_table_is_a_plain_list_of_numbers_for_the_state() -> None:
    ...
    Это тройки «сезон, серия, номер файла» и ничего больше: состояние - файл на диске, и
    класть в него объекты разбора нельзя, а номер файла обязан быть тем же, что в раздаче.
    """
