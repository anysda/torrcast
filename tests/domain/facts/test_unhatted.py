...
def test_a_pointer_joined_to_the_article_does_not_erase_its_description() -> None:
    """При потерянном переносе длинный абзац уже содержит паспорт самой картины."""
    paragraph = (
        "Не путать с одноимённым романом и его поздними экранизациями. "
        "«Возвращение» (англ. The Return) - художественный фильм 2003 года, "
        "рассказывающий о встрече двух братьев с отцом после долгой разлуки. "
        "Картина снята на натуре; главные роли исполнили молодые актёры, "
        "для которых эта работа стала дебютом в полнометражном кино."
    )
    assert unhatted(paragraph + "\n\nСюжет начинается летом.") == (
        paragraph + " Сюжет начинается летом."
    )
