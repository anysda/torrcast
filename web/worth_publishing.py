...
Планка полноты ТЗ §9 (:data:`web.min_tiles.FLOOR`) защищала старое тело условием
``min_tiles(current) >= FLOOR > min_tiles(best)`` - и держалась, пока обе полки сами
держали планку. После отсева неиграющих плиток (:mod:`web.shelf_playable`) настоящие
полки короче планки ВСЕГДА, и это условие не срабатывает больше никогда: одна неудачная
фоновая сборка (TorrServer лёг, индексер замолчал) молча перезаписала бы рабочую полку
пустой. Защита здесь читает ПРЕЖНЕЕ тело, а не снятую планку - ``FLOOR`` в этом модуле
не участвует и не импортируется (он остаётся картой TC-1353).
"""
