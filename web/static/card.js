...
const TCCard = {
  ...
  async _load(root, key, query, quiet, facts, season) {
    ...
    for (let turn = 0; ; turn += 1) {
      ...
      // застывшими до перезагрузки (замер на живом приёмнике, TC-1240).
  ...
  // не после круга поиска (холодная карточка на живом приёмнике ждала его 5.4 с).
  ...
  _body(data, key, query) {
    ...
    // стояла в `else`, она не рисовалась сериалу вовсе. На живом приёмнике 10-09-2026 это
  ...
  _buttons(data, key, query, isShow) {
    ...
    if (!noReleases && data.tv) {
      ...
      // отличие от соседних «Играть» и «Сначала» (замер на живом приёмнике 07-09-2026).
  ...
  _play(data, key, query, voices, fromStart, season, episode) {
    ...
    return TCApi.play({
      ...
      // зовётся запросом, у которого в круге две картины (замер на живом приёмнике 07-09-2026).
