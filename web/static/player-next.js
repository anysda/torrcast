// Плашка автоперехода (ТЗ §4.5, §11 п.8): за десять секунд до конца серии - «Смотреть»/
// «Отмена», сама играет счётчик. Плеер решает, КОГДА её показать; сама плашка решает
// только, что делать по истечении счётчика или по клику. Счёт всегда честный: либо сама
// досчитывает до нуля, либо человек снимает её кнопкой - середины нет.
'use strict';
...
const TCPlayerNext = {
  ...
  mount(root, onPlay, onCancel) {
    ...
    // Досчитала сама - карточка уходит тем же `stop()`, что и по кнопке: раньше висела
