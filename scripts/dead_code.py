...
#: Пакеты дерева. Три из них со своей точкой входа в `[project.scripts]`: показ
#: (`cast`), мост Home Assistant (`torrcast-ha`) и бот (`torrcast-bot`). Охват тут ВСЕ
#: по той же причине, по какой в нём лежат `scripts`: мост и бот - законные вызывающие
#: пакета, и без них живой код читается мёртвым. Замерено: единственный вызывающий
#: постера карточки живёт в `hass/`, и при охвате из одного `torrcast` вся цепочка за
#: постером (`WikiPoster`, `frame_shot`) объявлялась мёртвой, будучи живой. Своей точки
#: входа у `web` нет и не будет: страницу поднимает тот же мост (`hass/serve.py`), и
#: невнесённый сюда пакет был бы сторожем, отвечающим «мёртвого нет» там, где он не
#: смотрел вовсе.
PACKAGES = ("torrcast", "hass", "tgbot", "web")
