"""Карта опорных кадров mkv по HTTP — отчёт по файлу перед нарезкой.

Разбор Cues живёт в пакете (:mod:`torrcast.mkv`): по этой же карте показ строит сетку
...
from torrcast.mkv import Point, keyframes, video_track
