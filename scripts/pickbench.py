...
поднятое убирается из TorrServer ПО ЯВНЫМ ХЭШАМ - списком сервер чистить нельзя, наши
раздачи заведены с ``save_to_db: false`` и в списке не видны, а видно там чужое.
...
def once(url: str, order: list[cli._Plan], think: float, spare: bool) -> tuple[float, str]:
    ...
    bench.drop_all()  # по явным хэшам: у прогрева save_to_db выключен, списком не найти
    return spent, note
