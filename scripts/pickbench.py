...
:meth:`torrcast.usecases.select_bench.bench.Bench.resolve` - от «картина выбрана» до «отбор релиза».
Индексеры при этом не спрашиваются вовсе: план собирается из magnet-ссылок, названных в командной
...
(:data:`torrcast.domain.prewarm_settings.PREWARM`), то есть воспроизводят настоящую конкуренцию за
рой. ``--ceiling`` - потолок отбраковки, Мбит/с: между настоящими битрейтами двух релизов он даёт
сценарий «верх забракован», выше обоих - «верх годен».
...
что в списке лежат ЧУЖИЕ раздачи. Свои в нём тоже видны, но список не различает владельцев.
