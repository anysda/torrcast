...
* **SD наверху** - верх очереди отбора, про разрешение которого имя молчит, при живом
  ...
  (:func:`torrcast.usecases.rank.understated.understated`), а не ворота. Контрольный счёт
  «датированный верх при живом HD ниже» обязан быть нулём: его держит ступень
  :func:`torrcast.usecases.rank.is_dated.is_dated`.
* **«не знаю» против «мало»** - сериальные раздачи, чьё имя серий не считает
  (:func:`torrcast.usecases.rank.bitrate_of.bitrate_of` отдаёт ``None``): сколько их в очередях и
  верхах, и где ворота читают это молчание как вес.
* **тяжёлые приложения** - раздачи с меткой приложения в имени, которые ворота
  пропускают из-за веса (:func:`torrcast.usecases.rank.is_extra.is_extra` судит имя ВМЕСТЕ с
  битрейтом): сколько их, насколько они тяжелы и как высоко стоят.
