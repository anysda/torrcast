...
:data:`torrcast.adapters.recode.presets.PRESETS` сняты на одной машине и на такой же должны
пересниматься.
...
def calibrate(
    ...
    Профиль тяжести (:class:`torrcast.adapters.recode.weights.Weights`) считается по карте опорных
    кадров до первого сегмента, и на нём стоит решение «класть копией или перекодировать». Байты
    карты — контейнер целиком, поэтому из них вычитается поправка «контейнер → ТВ»
    (:attr:`~torrcast.adapters.recode.weights.Weights.extra`), а она до первой калибровки известна
    только паспортом ffprobe (:func:`torrcast.adapters.stream_pack.extra_mbit.extra_mbit`) — и
    известна **одним числом на весь фильм**.
    ...
    сдвинулась бы поправка (:meth:`torrcast.adapters.recode.weights.Weights.calibrate`). Отсюда и
    виден ответ на оба вопроса карточки: врёт ли прикидка одинаково по всему фильму и на каком по
    счёту куске поправка перестаёт ходить.
    """
    media = probe(url)
    keys = film_keys(url)
    grid = grid_for(
