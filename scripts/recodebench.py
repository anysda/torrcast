#!/usr/bin/env python3
"""Замеры под динамический битрейт (§6.2 SPEC-v2): скорость кодека и профиль тяжести.
...
``--speed ФАЙЛ``
    ...
    замер не лезли. Замер на стенде CT501 (4 vCPU E5-2696 v4, вход 23.7 Мбит/с, кап 12/13):
...
⚠️ Замер скорости не имеет смысла на машине с другим числом ядер: числа в
:data:`torrcast.recode.PRESETS` сняты на стенде и на нём же должны пересниматься.
"""
