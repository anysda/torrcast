...
Часть установки: его ставит и запускает `install.sh`, когда видит, что API отвечает
не целиком.

Бывает, что канал режет соединения по имени в SNI: с `api.knaben.org` заголовки
приходят, а тело обрывается на 16–20 КБ и висит (мелкие ответы проходят, крупные — нет).
Тот же IP того же Cloudflare с другим SNI отдаёт мегабайт за 0.38 с — то есть это не
MTU, не HTTP/2 и не сторона Knaben.
