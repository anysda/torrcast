...
def resume_state(state: Path, source: Path, key: str, position: float) -> str:
    ...
    путём («Продолжить? [Y/n]»), а не через поиск и меню.
    """
...
def watch_segment(
    ...
    (:meth:`torrcast.adapters.stream_pack.packer.Packer.publish`). Это и есть «готовность LOAD»:
    ...
    его печатает метка захода упаковки (:func:`torrcast.adapters.stream_pack.pack_start.pack_start`).
    """
    slot = 0
