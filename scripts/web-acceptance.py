...
#: 11-09-2026: 9, 20, 11 с (опрос `/api/state` раз в секунду во время каста на телевизоре).
...
#: Каст на ТВ судится после 15 с на подъём: приёмник сперва буферизует (живой приёмник
...
def _receiver_volume(ctx: Ctx) -> float | None:
    ...
    способом, что и показ. Другого способа честно снять громкость с телевизора со стороны
...
def check_12_franchise(base: str) -> Result:
    ...
    десять из десяти (живой приёмник, 10-09-2026, три прогона подряд), и допуск снят.
...
#: курсором, а вторую, часто вовсе за краем окна (живой замер, «Позвоните моему агенту»).
...
#: холодную, а её тёплого двойника с другой полки (замер на живом приёмнике, «Беги, прячься, бей»).
...
#: починки к 15-й секунде после загрузки было согрето 3 из 8 видимых плиток (замер на
#: живом приёмнике, окно 1920x1080), и последняя видимая открывалась холодной за 5.4 с.
...
def _tile_by_key(ctx: Ctx, key: str, group: str = "") -> Any:
    ...
    никогда (замер на живом приёмнике: 0 совпадений против 2 у той же плитки без `json.dumps`).
    ...
    на живом приёмнике: `strict mode violation` на «Позвоните моему агенту»), а `.first` без разбора
    иногда брал ТЁПЛОГО двойника вместо именно той холодной плитки, что искал вызывающий
    (замер на живом приёмнике: «Беги, прячься, бей»). Полка снимает разночтение; без неё - `.first`.
    """
...
def _cast_from_card(ctx: Ctx, button: Any) -> tuple[bool, str]:
    ...
    # касте (живой приёмник: пара 5.7 -> 5.7, а журнал остановил показ на 0:00:09).
...
#: кусок, а до живой приставки доходит 12.0 Мбит/с.
...
#: слышит рывками до 11 с, и ожидание его проскочило кусок 54.8 с на 0.3 с (живой приёмник).
...
#: (15 с на подъём и 40 с хода), а доигравшая серия уходит в следующую (живой приёмник
...
def check_35_same_on_tv(ctx: Ctx) -> Result:
    ...
    кусок. На своей упаковке вкладки (s1e2 сериала, живой приёмник: без перекода, куски
