...
# `requests.Response._content` читает сам `requests`: свойство `text` берёт его через
# `content` и ничего другого не спрашивает. Подделка отказа выставляет поле, потому что
# публичного способа положить тело в готовый ответ у библиотеки нет, а разбор отказа в
# `prowlarr_http_client` смотрит именно на `exc.response.text`.
_._content

# Те же обработчики сервера, что и выше, но у моста Home Assistant (`hass/serve.py`):
# чужой метод он отбивает своим ответом, а не молчанием стандартной библиотеки. Зовёт их
# ровно тот же `handle_one_request` по имени команды запроса.
_.do_DELETE
_.do_PATCH

# `sys_version` читает сам `BaseHTTPRequestHandler`, собирая заголовок `Server`. Мост
# ставит его пустым намеренно: версия питона в ответе - это подсказка чужому, а не
# сведения для карточки.
_.sys_version
