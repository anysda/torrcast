...
404 в отчёте — провал: живой ресивер, поймав его, не берёт LOAD ещё пару минут (§6
SPEC-v2), поэтому «файла не будет» показу разрешено отвечать только за концом фильма.

Сценарии (``--case``):

* ``back`` — упаковка от нуля, показ уходит вперёд за окно ``keep``, и с этого места
  запрашивается сегмент в самом начале фильма. Это 🔴 из §7.4: раньше запрос висел
  ``wait`` секунд и кончался 404, потому что «ниже ``packer.first``» считалось «вот-вот
  допакуется»;
...
def case_back(user: Consumer, keep: float) -> None:
    """🔴 §7.4: упаковка от нуля, показ ушёл вперёд, запрос — в самое начало фильма."""
...
def case_fwd(user: Consumer) -> None:
    ...
    for slot in range(far + 1, far + 4):  # префетч живого Q70D: шесть кусков разом
        user.take(slot)
...
def trace_steer() -> None:
    ...
    гадание, а гадать на этом месте уже пробовали (§7.4 SPEC-v2, откат de1ccc9).
    """
    raw = Feed._steer
