...
Адрес называет язык продукта. С `rutorrcast.anysda.space` встаёт русский `cast`: и надписи,
и предпочитаемая озвучка русские. С `https://torrcast.anysda.space` - английский. Выбор не
навсегда: `cast --ru` и `cast --en` переключают уже поставленную копию в любой момент.
...
Ключ называет язык: `-ru` ставит русский продукт, `-en` - английский. Без ключа
установка берёт английский, поэтому в русской установке ключ обязателен.
...
Итоговый экран установки говорит про приёмник сам. Записанный назван именем и адресом,
а если устройство имени не назвало (и на повторной установке, где в конфиге лежит один
адрес) - только адресом. Найденных перечисляет столько, сколько влезает под лого, и
считает остаток: «и ещё 2». Пустой поиск не остаётся молчанием: экран говорит, что
телевизор надо включить, и называет `cast --tv`; на узком терминале надпись сжимается,
но команда из неё не пропадает. Про mock-стенд сказано отдельно: каста наружу у него
нет. Там же названа дверь в другой язык - `cast --en` в русской установке.
...
cast --ru            # весь продукт по-русски: надписи, ответы бота, озвучки
cast --en            # весь продукт по-английски
cast -tg             # открыть меню настройки Telegram-бота
cast -h              # краткая форма справки
cast --help          # справка и полный набор ключей
cast --version       # версия
```

`sNeM` - часть позиционного запроса, а не ключ: `s2e5` означает сезон 2, серию 5.
Равнозначные формы - `2x5` и `2 сезон 5 серия`.
...
cast <запрос> --voice СТУДИЯ      # играть названной студией и запомнить
