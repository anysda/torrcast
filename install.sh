...
# сериалов нет). Опорные (:data:`CORE_INDEXERS`) сюда не попадают никогда: их непроход
# обязан быть виден в самой установке словами, а не найден потом в журнале.
...
# находит» - разные утверждения, но ждать второго человеку незачем. Опорных два, и оба
# проверяются на глазах (:func:`catalog_gate`: без любого из них поиск почти пуст, а без
# обоих пуст у 97 запросов из 99), остальные - здесь.
