...
TS_MEM_OVERHEAD=2
#: Насколько сборщику Go позволено растить кучу СВЕРХ живого кэша, процентов.
#:
#: Делитель выше долго был пожеланием: его вычитали из памяти машины, а самой службе
#: не говорили ничего, и Go по умолчанию (GOGC=100) удваивал кучу над живым кэшем -
#: ровно тот двукратный перерасход, под который делитель и заводился. Замер на стенде
#: 06-09-2026, один и тот же прогрев одной серии, кэш в памяти 3.13 ГиБ, потолок
#: контейнера 8196 МиБ: без ручки RSS службы 5911 МиБ и контейнер 8131 МиБ (99.2 %
#: потолка, 876 событий реклейма); с GOGC=25 - 4039 МиБ и 7367 МиБ (89.9 %, реклейма
#: ноль). Прогретое при этом совпало кусок в кусок: 119 кусков и один и тот же список.
#: 25 - это перерасход 1.25 при делителе 2, то есть запас ещё вдвое; GOMEMLIMIT ниже
#: не опускается и остаётся аварийным упором, а не рабочим режимом.
TS_GOGC=25
...
install_torrserver() {
    ...
    # Три потолка вокруг кэша, и ни один не для красоты.
    # GOGC - рабочий: он и держит перерасход над живым кэшем у делителя, которым кэш
    # размерен (TS_MEM_OVERHEAD). Без него Go растит кучу вдвое над кэшем, то есть ровно
    # до бюджета службы, и на памяти машины не остаётся ничего: замер на 8 ГиБ показал
    # 5911 МиБ у службы при кэше 3.13 ГиБ и 99.2 % потолка у машины.
    # GOMEMLIMIT - аварийный: он не запрещает расти, а заставляет сборщик работать
    # непрерывно у самой черты. Ставится по бюджету службы: рабочий режим до него не
    # доходит, а кэш, выросший сам, упирается в него, а не в память машины. С кэшем на
    # диске бюджет не зависит от размера кэша вовсе (замер: 104 МиБ при 3.2 ГиБ и 111 МиБ
    # при 12 ГиБ), поэтому там он фиксированный - TS_DISK_MEM.
    # MemoryMax - жёсткий, на случай, когда мягких не хватило (чужая утечка, раздача с
    # гигантским куском). Служба тогда умирает и поднимается заново
    ...
    local memory_knobs="Environment=GOGC=$TS_GOGC
...
ui_drain() {  # вычитать всё, что приехало в канал; без единого форка
  ...
  while IFS= read -r -u "$UI_CFD" line; do
    tag=${line%% *}; rest=${line#* }
    case $tag in
      # 🔴 TC-922. В след едет ПАРА «деление, имя фазы» - та самая, которую
      # человек читает в строке статуса («фаза 3/6: источники»). Мера, читающая
      # из следа один номер, держит связь «номер -> фаза» своей таблицей, и стоит
      # порядку `phase_done` в `main` разойтись с таблицей, как она сверяет время
      # одной фазы с работой другой - молча и зелено. Кадр для этого не годится:
      # два закрытия внутри одного кадра оставляют на экране только последнее.
      P) UI_DONE=$(( UI_DONE + 1 )); UI_PHASE=$rest
         ui_trace "CLOSE done=$UI_DONE total=$UI_TOTAL phase=$UI_PHASE" ;;
