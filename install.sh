...
# единственный найденный в сети берётся без вопроса, из нескольких она спросит номер.
#
...
# сети - он и есть телевизор, и берётся без вопроса; несколько - установка спросит
# номер, терминал у неё на этом месте есть. Никого не нашли (телевизор выключен или в
...
setup_receiver() {
    ...
    if TORRCAST_CONFIG="$cfg" "$BIN_DIR/cast" --tv; then
        return 0
    fi
    info "приёмник не настроился - причина в строках выше; включи телевизор и повтори \
./install.sh - он подхватит его сам"
}
