...
class HitPosters:
    ...
    def read(self, name: str) -> tuple[bytes, str] | None:
        ...
        байтах на диске (TC-1029). Наготове мост держит :data:`_KEEP` картинок, а имя
        выдаётся по полке - той самой, которую этот маршрут не спрашивал вовсе. Список
        длиннее памяти вытеснял из неё раньше выданные имена, и они отвечали 404
        МГНОВЕННО: ни памяти, ни ожидания. Отказ выглядел промахом байтов и лечился бы
        сроками, хотя в сеть при нём не ходили ни разу.
        """
        with self._lock:
