...
def _kind(shown: PlaybackSnapshot) -> str:
    ...
    новеллам («Аниматрица»), несёт ту же подпись `s1e1`, что настоящий сериал - замер
    TC-1050 теряет так постер у 3 антологий из 3. Спросить оба рода нашёл бы обе, но
    стоит лишнего похода в сеть на каждом показе - это цена, а не решение этого файла.
    """
