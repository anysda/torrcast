...
def searching(
    ...
    (:class:`hass.hit_posters.HitPosters`: статья найдена и год сверен) - не каждой находке,
    иначе список нёс бы рамку вокруг пустоты. Приговор ждётся тут же, в сети, до возврата
    списка: пачка стоит на нём один-два запроса разом, а не по три на находку, и это
    осознанный размен - плитка не бывает битой ценой этого ожидания. Не названный зовущим,
    он берётся из :data:`OFFER` в момент вызова, а не в момент объявления: подделка в
    зеркале ставится именно туда.
    """
