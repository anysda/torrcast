...
class Motion:
    ...
    def phase(self, shown: PlaybackSnapshot | None, *, active: bool, starting: bool) -> str:
        ...
        🔴 А вот БЕЗ юнита ``torn`` не бывает вовсе. Темнота переживает свой показ в
        записи нарочно - ``cast status`` ею рассказывает, чем кончился прошлый сеанс, -
        но карточке то же слово говорит другое: ``torn`` она рисует колесом «ещё на
        подходе». Поднимать погасший показ некому, и колесо над мёртвой записью стоит
        вечно: показ, не поднявшийся с первого раза, оставлял карточку в ``torn`` до
        следующего запуска, а остановка на ней отказывала (TC-1022, живой замер
        03-09-2026). Юнита нет - показа нет, и слово тут ``idle``, ровно то же, каким
        отвечает ``cast stop`` («ничего не играет»): две стороны продукта на один вопрос
