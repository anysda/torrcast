...
def pick_video_file(files: list[TorrFile]) -> TorrFile:
    ...
    Тип отказа здесь - не украшение, а решение отбора
    (:func:`torrcast.usecases.select._verdict._silenced`): :class:`InfraError` - это «рой промолчал,
    про раздачу не узнали ничего», и такую раздачу промолчавшая очередь спрашивает ещё раз. А тут
    метаданные приехали целиком и ответ известен навсегда: видеофайла в раздаче нет. Второй спрос
    дал бы ровно тот же ответ за те же секунды - как у «нужной серии в раздаче нет»
    (:meth:`torrcast.domain._series._Series.choose`), и тип у них один.
    """
