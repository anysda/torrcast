...
def probe(
    ...
    *,
    ...
    entries = (
        ...
        # Теги дорожки берутся ЦЕЛИКОМ, а не списком: если BPS лежит в голове, mkvmerge
        # пишет его то как ``BPS``, то как ``BPS-eng``/``BPS-rus`` - суффикс заранее неизвестен.
        # За хвостовыми статистическими тегами не идём: безопасная оценка веса по размеру
        # файла уже живёт в :meth:`Media.weight_mbit`, а языки и названия лежат в Tracks.
        "stream_tags"
    )
    # TorrServer умеет Range, поэтому обычный ffprobe считает вход перематываемым и у
    # mkvmerge-файла прыгает за Cues/Tags в последний кусок раздачи. Для паспорта нужны
