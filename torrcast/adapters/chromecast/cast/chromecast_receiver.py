...
class ChromecastReceiver(_Talk):
    ...
    def seek(self, pos: float) -> None:
        ...
        соединение тем же сендером (докстринг класса). Цель команды помнится только для
        отличия зрительской перемотки от скачка восстановления; ход показа и счётчики
        подвиса (``_peak``, ``_stall_hits``) намеренно не сбрасываются.
        """
        self._device().media_controller.seek(pos)
        # MEDIA_STATUS не называет отправителя SEEK. Опрос увидит только новую позицию,
        # поэтому сохраняем известную зрительскую команду до первого прыжка.
        self._viewer_seek_to = pos
