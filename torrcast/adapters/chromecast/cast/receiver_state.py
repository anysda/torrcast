...
class _State(_Settings):
    ...
    def __init__(
        ...
        #: Соединение с этим приёмником было живым хотя бы раз за показ. Признак липкий:
        #: :meth:`_Talk._restart_app` гасит само соединение (:attr:`_cast`), но факта, что
        #: приёмник в сети ЕСТЬ, не отменяет. По нему :meth:`_Link._no_link` и выбирает,
        #: чем отвечать на несостоявшийся коннект: отсутствием приёмника или отказом
        #: загрузки, который показ переживает (TC-916).
        #:
        #: Стенд, назвавший своё соединение параметром, тоже уже связан: спрашивать
        #: с него первый коннект нечем, он его не делал.
        self._linked = device is not None
        self._url = ""
        self._title = ""
        self._peak = 0.0
        self._stall_at = -1.0
        ...
        self._seen_at = 0.0
        #: Состояние из того же снимка, что :attr:`_seen`: скачок после уже замеченного
        #: ``BUFFERING`` приёмник делает сам, возвращаясь после ребуфера.
        self._seen_state = ""
        self._seek_from = 0.0
        self._seek_to = 0.0
        ...
        self._nudged_to = -1.0
        #: Куда велел перемотать наш владеющий сендер (страница, HA или сухой пульт).
        #: В ``MEDIA_STATUS`` источник команды не приезжает, поэтому держим его здесь
        #: до первого прыжка; несовпавший прыжок тоже гасит метку.
        self._viewer_seek_to = -1.0
