...
class _Settings:
    ...
    #: показа. Опрос идёт раз в 2 с (:func:`torrcast.usecases.revive_playback._hold._hold`), и за
    #: него живой показ уходит на те же 2 с; берём с запасом на подвисший опрос. Собственный нудж
    #: под этот порог тоже попадает, поэтому он помечается отдельно (:attr:`_nudged_to`).
    SEEK_JUMP = 15.0
