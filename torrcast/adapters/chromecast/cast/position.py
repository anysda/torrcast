...
def _position(rcv: _Talk, front: float = 0.0) -> Position:
    ...
    if pos > 0.0 and state not in ("BUFFERING", "IDLE"):
        ...
        rcv._shown = pos
    # Пауза - решение ЗРИТЕЛЯ, и признак её обязан быть наш: слово приёмник теряет
    # вместе с сессией, а память о паузе теряться не вправе. Взводится и гасится он
    # ровно теми же двумя случаями, что и у круга опроса показа
    # (:mod:`torrcast.usecases.revive_playback._paused`): пауза взводит, живой экран
    # гасит, потеря сессии не трогает ни того, ни другого.
    if state == "PAUSED":
        rcv._paused = True
    elif st.player_is_playing:
        rcv._paused = False
    if state == "PLAYING":
