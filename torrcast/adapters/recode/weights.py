...
class Weights:
    ...
    @classmethod
    def flat(cls, count: int, delivered: float) -> Weights:
        """Профиль без карты: один и тот же вес на каждый кусок, по паспорту ffprobe.

        Карты нет вовсе (индекс контейнера врун или его нет) или она без смещений — тогда
        про КАЖДЫЙ кусок известно ровно одно: средний вес фильма. Это грубо и нарочно
        грубо: ровный профиль не отличает тяжёлое место от лёгкого, поэтому он либо
        объявляет тяжёлыми все куски, либо ни одного. Зато он честен там, где карты нет,
        и ровно он превращает «ужимать нечем» в «ужимаю».

        ``delivered`` — сколько Мбит/с уедет на ТВ в среднем по фильму
        (:attr:`torrcast.domain.media.Media.delivered_mbit`). Ноль — паспорт молчит, и
        профиль выходит нулевым: тяжёлых кусков он не назовёт ни одного, но кодировщик
        всё равно поднимется, и слишком увесистую копию будет чем ужать на месте
        (:func:`torrcast.usecases.feed_pack.feed_shrink._shrink`).

        Поправка «контейнер → ТВ» тут нулевая по построению: ``delivered`` — это уже вес
        того, что уедет, а не контейнера. Уточнять её по факту всё равно есть чем
        (:meth:`calibrate`).
        """
        mbit = max(0.0, delivered)
        return cls(raw=(mbit,) * max(0, count), extra=0.0, measured=PASSPORT_WEIGHT, container=mbit)
