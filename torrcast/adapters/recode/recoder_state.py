...
class _State:
    ...
    edge: int = -1
    #: Чем фактически ушли слоты этого показа. Помнит не план, а выкладку: после
    #: перемотки ей же отвечает keyless-копия, чей левый сосед уже опубликован.
    ...
    def working(self) -> bool:
        ...
        теряет 30 % скорости (замер соседства - :data:`COPY_TOLL`), а ``nice`` от этого
        не спасает -
