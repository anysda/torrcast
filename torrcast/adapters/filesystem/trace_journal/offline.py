...
def offline(why: str, asked: bool = False) -> None:
    ...
    ``asked`` - правда ли причину назвал сам источник
    (:meth:`torrcast.adapters.stream_probe.supply.Supply.check`), а не догадка по мёртвому прогону
    упаковки. Разница существенная: «упаковка оборвалась» и «служба раздач не отвечает» выглядят в
    показе одинаково, а значат разное, и в следе это должно быть видно без гадания.
    """
