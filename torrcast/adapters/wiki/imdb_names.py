...
class ImdbNames:
    ...
    def _shorn_id(self, title: str, year: int | None) -> str:
        ...
        и хвост не отбрасывается. Слова берутся из сведённого имени, а не из ``split()``:
        знак препинания без пробела («Классический-Доктор Кто») склеивал приставку с именем
        в одно слово, и страж не пробовал ни разу, хотя сведённое имя было то же самое.
        Спрашивается только имя, которого карта не знает целиком: неоднозначное имя так и
        остаётся неоднозначным. Приставка не длиннее самого имени, имя короче двух слов
        приставкой не добывается («Пыльная работа» - не «Работа»), и год должен оставить
        единственный сериал: иначе по-прежнему пусто.
        """
