...
def _unmeasured(source_url: str, why: str) -> float:
    ...
    посадки ровно на кадр. Сверка судит его по-прежнему как посадку на границе, но полка
    не вправе назвать такой запуск измеренным и сохранить его для следующего показа.
    """
