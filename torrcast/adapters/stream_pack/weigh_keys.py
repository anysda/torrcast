...
def weigh_keys(cache: Path) -> FilmKeys | None:
    ...
    🔴 После того как номер стали спрашивать ОБА соседа - и карта, и вердикт
    (:func:`~torrcast.adapters.stream_pack.refused_keys.refused_keys`), - слепота этого
    читателя проверена заново и оставлена нарочно. Запись чужих правил сюда доходит ровно
    в одном случае: разбор, поднятый её же пересудом, не кончился ни картой, ни новым
    вердиктом - то есть промолчал рой
    (:class:`~torrcast.domain.swarm_silent_error.SwarmSilentError`). Спроси мы номер и тут -
    показ на молчащем рое лишился бы и веса, который у него уже есть на руках, за чужую
    беду. Байты в такой записи от смены правил не портятся: их писал разбор кластеров, а
    не проверка кадров.
    """
