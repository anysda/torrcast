...
def _legacy_ffmpeg_pack_command(
    ...
    * ``-readrate_initial_burst`` (ffmpeg ≥ 6.1) - первые ``burst`` секунд читаются на
    ...
    ``encode`` (:class:`torrcast.adapters.recode.encode.Encode`) заменяет ``-c:v copy``
    перекодированием тяжёлого куска. Всё остальное - сетка, метки, границы, звук - остаётся тем же,
