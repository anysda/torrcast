...
def extra_mbit(keys: FilmKeys, delivered_mbit: float) -> float:
    ...
    Ровно то же число, что набирает :meth:`torrcast.adapters.recode.weights.Weights.calibrate` по
    факту, но известное до первого куска. Паспорт молчит (mp4 без тегов) — ноль: тогда потолок веса
