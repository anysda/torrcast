...
def grid_for(
    ...
    *,
    ...
    try:
        ...
        # 🔴 Карты для РЕЗА нет, но байтовый указатель для ВЕСА рядом с вердиктом бывает
        # (:func:`weigh_keys`), и путей к нему два. Второй показ фильма, чью карту отверг
        # первый, - и ПЕРВЫЙ показ файла, чей индекс разобрался, но признан призрачным
        # (:class:`~torrcast.domain.ghost_keys_error.GhostKeysError`): там отвергнуты кадры, а
        # байты честные. Без указателя ровная сетка остаётся без профиля тяжести, и каждый
        # её кусок идёт ужатием на месте.
        return replace(
