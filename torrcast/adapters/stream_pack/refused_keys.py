...
def refused_keys(cache: Path, ttl: float) -> str | None:
    ...
    переполняют полку - её потолок считает файлы, а не ответы, - и лишнего места не
    занимают: вердикт весит сотню байт против десятков килобайт карты. Карту с такой
    записи не прочитать (:func:`~torrcast.adapters.stream_pack.read_keys.read_keys` не
    найдёт в ней точек и промолчит), и это ровно то, что нужно.
