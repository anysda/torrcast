...
class _State:
    ...
    #: Дальше этот огрызок лежал в :data:`torrcast.adapters.recode.recode_dir.RECODE_DIR` как готовый
    #: кусок и выкладывался наружу вместо честной копии: живой Q70D на «Тачках 3» - ``v311`` 1.3 МБ
    #: вместо 11 МБ, приёмник встал на 8 с и потерял 8 секунд фильма. У «Моаны 2» это не всплывало:
    #: там тяжелы почти все куски подряд, и огрызок всегда успевал смениться настоящим перекодом
    #: следующего захода.
    last: int = -1
