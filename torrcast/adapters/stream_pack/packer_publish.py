    ...
    *,
    ...
    for slot in done:
        ...
        if slot < state.first or 0 <= state.last < slot:
            ...
            continue
        # Кусок сейчас перекодируют - подождём его (:attr:`hold`): дыра увела бы край,
        # а запрос придержанного места выглядел бы для :meth:`Feed._steer` перемоткой назад.
