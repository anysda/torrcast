...
def refuse_keys(cache: Path, refused: str, keys: FilmKeys | None = None) -> None:
    ...
    самом деле. Указатель приезжает сюда от ОБОИХ, и это не симметрия ради красоты: у
    разбора он есть всегда, когда индекс разобрался и признан призрачным, а молчание про
    него стоит зрителю ровно того же, что и молчание сетки. Не запиши мы вердикт второго,
    следующий показ того же фильма взял бы ту же карту с полки и построил бы по ней ту же
    сетку - полка помнит карту дольше, чем сеанс помнит, что она соврала.
