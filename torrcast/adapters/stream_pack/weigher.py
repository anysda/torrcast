...
def weigher(
    ...
    ``extra_mbit`` (:class:`torrcast.adapters.recode.weights.Weights` считает ту же поправку), а
    тяжёлый кусок ещё и перекодируется — выше ``ceiling_mbit`` он не уедет при всём желании.
