...
class Grid:
    ...
    #: прогрев проверяет по ней бюджет диска (:meth:`torrcast.usecases.warm.Warmer._forecast`).
    ...
    def on_keyframes(
        ...
        **если** предсказанный вес куска влезает в ``cap``; не влезает — последний кадр,
        ...
        (:class:`torrcast.adapters.recode.Weights`): ``sizes`` — смещения опорных кадров в файле,
    ...
    def after(self, seconds: float) -> float:
        ...
        споткнулся (:meth:`torrcast.adapters.chromecast.cast.ChromecastReceiver._nudge`): прыжок
        короче сегмента приземляется в него же и перешагнуть его не может никогда. Сегменты разной
        длины (6.0-14.9 с на «Моане» 2016), поэтому шаг тут и не может быть числом - только границей
        сетки.
        """
