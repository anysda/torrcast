...
#:
#: Разгадка в том, ЧТО именно ломается: в этом месте фильма лежит «тяжёлый» GOP —
#: 4.8 с и ~7.5 МБ между опорными кадрами. Приёмник переваривает его, только пока
#: граница сегмента его не трогает: любая граница внутри GOP (и даже ровно на его
#: опорном кадре) убивает показ, а целиком внутри одного сегмента тот же GOP играет.
#: Сетка 4 с такой GOP разрезает всегда — 10 с оставляет его целым.
#:
#: ⚠️ Это не «настроили покрасивее»: 10 с ровно настолько больше самого длинного GOP
#: этого релиза (11.5 с — верх, типовой 3–5 с), чтобы тяжёлые GOP не резались. Меньше
#: брать нельзя, а больше — дороже старт (первый сегмент надо допаковать целиком).
