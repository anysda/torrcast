    ...
    * запрос сегмента, которого нет, — это и есть перемотка. Показ не отвечает 404
    ...
    def _steer(self, slot: int) -> bool:
        ...
        if packer is not None and not packer.halted:
            ...
            # ⚠️ Нижняя граница именно `packer.first`, а не край упаковки, и это не
            # недосмотр. Ревью 06-08-2026 предложило ждать только ВПЕРЕДИ края (мол,
            # сегмент ниже края прогон уже отдал), и замер на стенде это опроверг:
            # `frontier()` считает край глобом каталога, где лежат и куски прошлых
            # прогонов, поэтому «ниже края» регулярно попадает то, что этот прогон ещё
            # не дописал. Со строгой границей приёмник, идущий быстрее упаковки, ронял
            # показ в перезапуск каждые 10 секунд: 20 перезапусков за 100 с против 1.
            # Чинить это надо с другого конца — с честного края прогона (см. SPEC-v2 §7.4).
