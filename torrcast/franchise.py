...
def confirmed_continuations(
    ...
    """Продолжения, названные подзаголовком БЕЗ двоеточия, за которые ручается каталог.

    Продолжение, чьё имя каталог ввёл не двоеточием и не номером, а тире, номером версии
    или просто словом («Евангелион 1.11», «Токийский Гуль ТВ-2», «Мумия возвращается»),
    получает СВОЙ ключ франшизы: ``евангелион-1-11`` рядом с ``евангелион``. Раскрытие
    такие ключи не берёт вовсе, и до меню картина не доезжает - на сохранённых выдачах
    треть запросов теряет так хотя бы одну свою часть.

    🔴 По имени этот класс не разводится, и пробовать больше незачем: любая форма ключа
    приводит в меню чужих однофамильцев, а порог в одну группу крадёт дефолты. Разница
    между «Мумия возвращается» и «Оно приходит ночью» на русском ключе не видна вовсе -
    обе строки начинаются именем спрошенной франшизы и продолжаются своими словами.

    Поэтому спрашивается не имя запроса, а ВТОРАЯ подпись каталога - оригинальное имя,
    которое в :attr:`Picture.original` попадает только из релиза, назвавшего оба имени
    разом. Продолжением признаётся картина, у которой сошлось всё перечисленное:

    * корень её оригинала ТОЧНО равен корню оригинала картины, уже стоящей в этой
      франшизе. Не вхождение и не общее слово: ``it`` входит подстрокой в
      ``it-comes-at-night``, и нестрогая ступень привела бы в «Оно» четыре чужих фильма.
      Точное равенство значит, что каталог сам поставил под этот корень картину
      франшизы, - за пару ручается его строка, а не наша догадка;
    * картина вышла НЕ РАНЬШЕ франшизы. Продолжения раньше начала не бывает, и это
      единственное, чем «Шерлок Бонд: Пес-детектив» (1994, оригинал ``Sherlock:
      Undercover Dog``, корень ``sherlock``) отличается от настоящего продолжения:
      корень у него сходится с корнем сериала «Шерлок», а год - на шестнадцать лет
      раньше;
    * первой строки меню она не занимает. «Дефолт франшизы - первая живая часть»: человек
      жмёт Enter, не читая списка, и пополнение меню не имеет права менять то, что при
      этом заиграет. Условие отдельное, а не следствие двух первых: «Евангелион
      по-новому» (2007, оригинал ``Evangelion: 1.0 You Are Alone``) - настоящее
      продолжение, корень и год у него сходятся, - но каталог назвал ему номер части, и
      в меню картина вставала ВЫШЕ прежней первой, забирая её дефолт себе.

    ⚠️ И-форма (``<франшиза>-и-<подзаголовок>``) сюда не входит: у неё свой порог в две
    группы, поставленный отдельным замером, и трогать его тут нечем.
    """
    ...
    if not found:
        return []
    # Первую строку меню считает :func:`menu_order`, а не порядок сортировки: номер части,
    # названный каталогом, поднимает картину выше более ранней безномерной. Поэтому верх
    # меряется дважды - до пополнения и после, - и пришелец, оказавшийся наверху, из
    # пополнения выбывает. Сместил верх не пришелец (пополнение пересобрало саму линейку)
    # - отказываемся от всего: дефолт дороже полноты меню.
