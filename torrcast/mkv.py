...
``SeekHead`` в голове. Замер 06-08-2026 на стенде: «Моана 2» (3.4 ГБ, 1119 опорных
кадров) — 0.45 МБ и 4.3 с холодным роем, «Моана» 2016 (4.5 ГБ, 2830 опорных) — 2.1 с.
"""
...
def _points(body: bytes, facts: _Head) -> list[Point]:
    ...
    смещение обязано быть абсолютным: по нему греется рой под перемотку (§7.2 SPEC-v2), а
    рою всё равно, что там за матрёшка, — он знает только байты от начала файла.
    """
