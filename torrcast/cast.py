...
и отдаёт позицию — на нём проходит вся автономная приёмка.
Samsung-специфики здесь нет и быть не должно: ни PowerState, ни nudge-сторожей.
"""
...
class ChromecastReceiver:
    ...
    def replay(self, at: float) -> bool:
        ...
        Терпение приёмника конечно и меньше нашего: у Q70D это ~4 минуты пустого экрана и
        два повтора LOAD, после которых он бросает показ насовсем. Дальше повторять LOAD
