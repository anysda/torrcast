...
class ChoiceEnvironment(Protocol):
    ...
    #: Картина, стоявшая под номером в последней показанной таблице запроса: её ключ и
    #: имя для человека. Пустая пара - таблицы не было, и сверять номер не с чем.
    def recalled_pick(self, query: str, number: int) -> tuple[str, str]: ...

    #: Запомнить порядок показанного списка картин: ключ и имя под их номерами. Пишется
    #: ровно то, что напечатано, - номер пункта обязан значить то же и в следующем запуске.
