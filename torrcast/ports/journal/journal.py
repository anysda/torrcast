...
class Journal(Protocol):
    ...
    def refetch(self, pos: float, tries: int, ok: bool, why: str = "") -> None:
        ...
        Событие отдельно от :meth:`reload`, и слить их нельзя: там медиасессия умерла и
        показ поднимают заново (счёт идёт по ``load_retries``), тут она жива и приёмник
        переспрашивает источник, не гася картинку (``segment_retries``). Общий счётчик
        сложил бы два разных отказа в одно число.

        ``ok`` названо отдельно от ``why`` нарочно: пустая причина у пустого поля значила
        бы и «перезабор удался», и «исход не назвали», а это разные новости.
        """
