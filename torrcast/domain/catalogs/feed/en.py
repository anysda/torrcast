...
def en() -> dict[str, str]:
    ...
    return {
        "feed.weight_mb": " ({mb:.0f} MB)",
        "feed.shrinking": (
            "v{slot} heavier than the cap{weight} - shrinking in place down to {mbit:.1f} Mbit/s"
        ),
        ...
        "feed.source_mute_reason": "the source has been silent for over {secs:.0f} s",
        ...
        "feed.pack_from": "packing from {start:.1f} s",
        "feed.catchup": " (catch-up {drop:.1f} s)",
        "feed.warm_torn": (
            "warmed v{slot} is cut short (missing {missing:.2f} s) - redoing with a live pack"
        ),
        "feed.warm_off_grid": (
            "warmed v{slot} is off the grid ({diff:+.2f} s) - redoing with a live pack"
        ),
        "feed.give_up": (
        ...
        "feed.pending_too_big": (
            "undelivered pieces are {mb:.0f} MB in memory - stopping the pack, "
            "will raise it on the receiver's request"
        ),
        "feed.pending_reason": "{mb:.0f} MB undelivered in memory",
