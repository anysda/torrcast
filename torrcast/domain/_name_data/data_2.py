...
#: Та же раскладка, но БЕЗ скобочной группы впереди: вместо неё признаком серии стоит
#: ВЕДУЩИЙ НОЛЬ. Разбор - в :func:`~torrcast.domain.fansub_episode._fansub_episode`.
#:
#: 🔴 Номер РОВНО двузначный. Трёхзначный «0dd» отсюда убран: он берёт «James Bond - 007»,
#: и раздача без года читалась бы серией, а год у неё есть не всегда. Замерено: на
#: замороженной выдаче и на 7008 именах из фикстур этой раскладкой взято 45 и 5 номеров,
#: трёхзначных - НОЛЬ, то есть сужение не теряет ни одной строки. Длинный сериал не
#: страдает: у `_FANSUB_EPISODE_RE` номер `\\d{1,3}` любой, и «[Erai-raws] One Piece - 007»
#: разбирается ею, - улику там даёт группа, и подпорка ведущим нулём не нужна.
_BARE_EPISODE_RE: Final = re.compile(
    "^(?P<name>[^\\[\\]()]+?)\\s+-\\s+(?P<episode>0\\d)"
...
#: Слова, которыми выдача называет ЭКРАНИЗАЦИЮ, но не форму: «Sakusei Byoutou The
#: Animation» и «Sakusei Byoutou» - одна и та же работа, а не полный метр против
#: сериала. В этом вся разница с `_FORM_WORDS`, и из-за неё список отдельный.
#:
#: Слово формы между ВИДАМИ - улика: «Naruto Shippuuden Movie» отличает от сериала
#: ровно оно, поэтому снимать его позволено только там, где вид уже сошёлся, - в ведре
#: склейки. Слово экранизации о виде не говорит НИЧЕГО: под ним лежат и сериал, и
#: полный метр. Поэтому оно снимается и в ключе франшизы
#: (:func:`~torrcast.domain.franchise_key.franchise_key`), где вида нет вовсе, - а без
#: этого склейка чинилась только наполовину: картины сходились, а франшизы оставались
#: две, и запрос по голому имени попадал в ту, где сидов нет.
#:
#: Записи многословные нарочно: голое «animation» стоит в живых именах само по себе
#: («Animation Runner Kuromi»). 🔴 Но многословность имя НЕ бережёт: у «The Animation
#: Runner Kuromi» ведущее «The» самого имени даёт полное совпадение. Имя бережёт только
#: то, что :func:`~torrcast.domain.adaptationless._adaptationless` снимает примету одним
#: ХВОСТОМ ключа; список закрыт, а позиция названа там.
_ADAPTATION_WORDS: Final = frozenset({"the-animation", "the-anime"})
...
_EPISODE_BRACKET_RE: Final = re.compile(
    ...
    # «(1-14 сезоны)» - линейка СЕЗОНОВ, и серии 1-14 из неё отсекали s3e20 целого пака.
    "\\s*-\\s*(?P<end>\\d{1,3})(?!\\d)(?!\\s*(?i:сезон|season))[^\\[\\]()]*[\\])]"
)
