...
def _formless(key: str) -> str:
    ...
    🔴 Если слово формы стоит ПЕРВЫМ, а после номера ничего не осталось, снимать
    нечего: перед словом нет имени-приставки, и голый номер стал бы именем картины
    целиком - «Movie 43» слился бы по ключу `43` с любой другой картиной того же
    вида. Такой остаток бережётся точно как пустой: ключ не отдаётся.
    ...
    for start in range(len(parts)):
        for size in (2, 1):
            ...
            if start == 0 and not tail and number:
                continue
