...
class _ConfigHls(_ConfigSources):
    ...
    #: Сколько секунд плёнки имеет право держать ВКЛАДКА. Запас на сервере и запас у
    #: зрителя - разные величины: вкладка берёт не сколько упаковано, а сколько ей
    #: разрешено, и дальше просто перестаёт просить куски. Замерено на живом показе:
    #: при 300 с готовой упаковки вкладка держала 34 с по медиане, а с поднятым потолком
    #: на той же картине и тех же ручках - 121 с.
    #: Умолчание равно умолчанию hls.js (``maxBufferLength``), поэтому ключ ничего не
    #: меняет, пока его не назвали.
    hls_tab_buffer: float = 30.0
    #: Потолок того же запаса по МЕСТУ, мегабайты (``maxBufferSize`` в hls.js). Наступает
    #: раньше секунд на плотной картине: 60 МБ на 8 Мбит/с - это всего минута.
    #: Умолчание тоже равно умолчанию hls.js.
    hls_tab_bytes: float = 60.0
