...
которую показ греет под меню всё равно (:func:`torrcast.adapters.stream_pack.warm_at.warm_at`), — то
есть ничего сверх. Замер на «Moana.2.2024.REPACK.1080p.BluRay.x264.AAC5.1-[YTS.MX]» (1.97 ГБ, 1304
опорных кадра, в рое один сид): три Range-запроса, 2.36 МБ, 15.9 с на холодной раздаче и 0.05 с,
когда те же байты уже в кэше TorrServer. Целиком ``moov`` там 5.26 МБ, но дорожка звука лежит за
дорожкой видео, и до неё дело не доходит.
