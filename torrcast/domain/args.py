...
class Args:
    ...
    telegram: bool = False
    #: ``--upgrade`` - обновить продукт до последней выпущенной версии. Это вся работа
    #: запуска целиком, а не оттенок другой: ключ не сочетается ни с запросом, ни с
    #: выбором телевизора, и названную рядом работу он вытесняет, а не дополняет.
    upgrade: bool = False
    ...
    #: Внутреннее: этот запуск играет у того, кто попросил показ, а не на
    #: ``Config.tv``. Ставит его страница браузера (:mod:`hass.play_extras`) через
    #: ``POST /api/play`` - руками не зовётся, как и ``play_key``.
    here: bool = False
    #: Внутреннее: ключ картины, открытой на карточке страницы. Картину называет он, а не
    #: номер в выдаче: номер гуляет от круга к кругу, ключ - нет.
    picture: str = ""
    #: Внутреннее: оригинальное имя картины карточки. Позднюю картину сериала показ
    #: принимает за ту же только при совпадении оригинала, иначе это ремейк.
    picture_original: str = ""
    #: Внутреннее: инфохэш раздачи, которую карточка уже отобрала. Отбор спрашивает её
    #: первой, а мёртвую меняет обычной очередью и говорит об этом строкой.
    card_release: str = ""
    #: Внутреннее: числа серий сезонов списка карточки «60,60,61,98», когда он нумерует
    #: сериал не как раздачи. Серию строки показ ищет тогда сквозным номером.
    layout: str = ""
    ...
    def command(self) -> str:
        ...
        ``configure`` / ``worker`` / ``upgrade``.
        """
        if self.play_key:
            return "worker"
        # Обновление стоит выше языка и запроса: `cast --ru --upgrade` - это просьба
        # обновиться, а не переключить язык, и запомненный язык обновление и так возьмёт
    ...
    def from_menu(self) -> bool:
        ...
        """
        return self.menu or self.pick is not None or bool(self.picture)
