...
class StartRefusedError(InfraError):
    ...
    (:meth:`torrcast.adapters.chromecast.cast.ChromecastReceiver._device`) - показывать некому и
    нечем, показ кончается тут же. Приёмник ЕСТЬ и отказал на загрузке - это не конец показа, а его
    первая смерть, и поднимается она ровно тем же путём, что и смерть посреди фильма
    (:class:`torrcast.usecases.revive_playback._Revival`).
