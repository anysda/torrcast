...
class StartRefusedError(InfraError):
    ...
    разные. Приёмника нет в сети
    (:meth:`torrcast.adapters.chromecast.cast.chromecast_receiver.ChromecastReceiver._device`) -
    показывать некому и нечем, показ кончается тут же. Приёмник ЕСТЬ и отказал на загрузке - это не
    конец показа, а его первая смерть, и поднимается она ровно тем же путём, что и смерть посреди
    фильма (:class:`torrcast.usecases.revive_playback._revival._Revival`).
