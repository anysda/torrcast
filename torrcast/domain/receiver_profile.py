...
class ReceiverProfile:
    ...
    #: Сколько секунд браузер вправе молчать позицией, прежде чем показ пометит её
    ...
    #: у него не реализован намеренно). ``0.0`` - приёмник смертью вкладки не мерен.
    lost_after: float = 0.0
    #: Сколько секунд браузер вправе молчать позицией, прежде чем сеанс закроют штатным
    #: :func:`torrcast.usecases.playback._show_end._close_show` - тем же путём, каким
    #: кончается потерянный телевизор. ``0.0`` - приёмник смертью вкладки не мерен.
    gone_after: float = 0.0
    ...
    #: не шлёт. ``0.0`` - приёмник таким словом не мерен.
    left_after: float = 0.0
