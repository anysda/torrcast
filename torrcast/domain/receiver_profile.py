...
class ReceiverProfile:
    ...
    blind_nudges: int = 3
    #: Достаточность роя до показа: запас над реальным временем и честное окно доставки.
    supply_ratio: float = 1.25
    #: Рой разгоняется 4.5--9.5 с, поэтому первые 10 с не судим ни у одного приёмника.
    supply_settle_seconds: float = 10.0
    #: Счётчик снимается лишь на трёх вехах прогрева; за 2 с после разгона следующая
    #: веха уже даёт честную скорость, не откладывая отсев слабого роя до 20-й секунды.
    supply_window_seconds: float = 2.0
