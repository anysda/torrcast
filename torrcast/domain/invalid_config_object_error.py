...
class InvalidConfigObjectError(TorrcastError, ValueError):
    ...
    Телеграм-бот же говорит языком ЗРИТЕЛЯ, и ему эту же беду надо назвать заново
    (:func:`tgbot.i18n._failure_detail`) - а собрать её заново можно только из пути,
    не из чужого готового текста.
