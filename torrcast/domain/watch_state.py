...
class WatchState:
    ...
    def held(self) -> set[str]:
        ...
        по устройству - юнит показа один на ЭКЗЕМПЛЯР
        (:func:`torrcast.domain.unit_naming.unit_name`), и новый ``cast`` гасит
        прежний до своего старта
        (:func:`torrcast.adapters.systemd.start_play_unit.start_play_unit`). Поэтому раздача либо
        чья-то, либо ничья, и спрашивать «сколько держат» не приходится.
...
def _other_part(query: str, want: str, key: str, entry: Entry) -> bool:
    ...
    Спрашивается при этом НОМЕР, а не имя: имя у записи и у запроса живут на разных языках («Moana
    2» под запросом «моана 2»), а номер части один на оба
    (:func:`~torrcast.domain.split_franchise_index.split_franchise_index`). Названный номер сошёлся
    с номером записи - назвали её и есть; номера нет ни там, ни там - речь об одной картине, и
    продолжение идёт как раньше. «Аполлон 13» на запрос «аполлон 13» тоже проходит: там номер
    назван, и он тот же самый.
    ...
    У сериала число в названии - сезон, а не соседняя картина: сезон ему называют
    только явно (``s6e1``, ``6x1``, «сезон 6 серия 1»), голое число в конце запроса
    сезоном не бывает никогда. Сезоном же число является, когда в нём стоит сама
    запись: «Кухня 6» с местом в s6 на запрос «кухня» продолжает свою серию, сколько
    бы серий ни оказалось в раздаче. Записи «фильм сериалом» («Moana 2», застрявшая
    в s1e1) это освобождение не касается: её двойка сезоном не является.
    ...
    if part is None or part == 1 or part == split_franchise_index(query)[1]:
        return False
    if entry.kind == "tv" and part == entry.season:
        return False
