...
class WatchState:
    ...
    def held(self) -> set[str]:
        ...
        прежний до своего старта (:func:`torrcast.adapters.systemd.start_play_unit.start_play_unit`).
        Поэтому раздача либо чья-то, либо ничья, и спрашивать «сколько держат» не приходится.
...
def _other_part(query: str, want: str, key: str, entry: Entry) -> bool:
    ...
    (:func:`~torrcast.domain.split_franchise_index.split_franchise_index`). Названный номер сошёлся с
    номером записи - назвали её и есть; номера нет ни там, ни там - речь об одной картине, и
