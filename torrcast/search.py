...
публичные, apikey локальный и генерится ``install.sh`` (§3 ТЗ). Модуль возвращает
сырую выдачу; смысл ей придаёт :mod:`torrcast.parse`.

⚠️ Две правки к §8 ТЗ, выясненные на живом Prowlarr 2.5.2: агрегат по всем
...
сами и вешаем публичные трекеры (§3: «пиры — magnet + DHT + публичные ретрекеры»).
"""
