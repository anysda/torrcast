...
class Prowlarr:
    ...
    def _apart(self, query: str, limit: int) -> list[RawResult] | None:
        ...
        # Полная страница - обрезанный хвост независимо от запрошенного ``limit``.
        # Опоздавшие сюда не попадают - их счёт неизвестен, а гадать нельзя.
        self.capped = tuple(name for name, rows in counts.items() if rows >= _INDEXER_PAGE)
