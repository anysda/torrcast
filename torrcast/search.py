...
⚠️ Четвёртая: круг ждёт опорные источники (:data:`_WAIT_INDEXERS`), а не последнего
ответившего. Опоздавший при этом не выбрасывается - он доезжает фоном и забирается
...
_FRAGILE_TIMEOUT: Final = 3.0
_FRAGILE_BUDGET: Final = ("nyaa", "rutor")
#:
...
_WAIT_INDEXERS: Final = (*QUORUM_INDEXERS, "rutor")
...
def indexer_budget(name: str) -> float:
    ...
    * КОРОТКИЙ СПИСОК (TC-213): у кого нечего терять, тот и ждать себя не заставляет
    ...
    fragile = any(mark in low for mark in _FRAGILE_BUDGET)
    if fragile:
        return min(role, _FRAGILE_TIMEOUT)
...
def response_budget(name: str) -> float:
    ...
    уничтожает ответ. Некворумные сохраняют свой личный срок; лишь Knaben может
...
def _wait_indexer(name: str) -> bool:
    """Нужен ли источник в первом пуле, даже если без него поиск способен жить."""
    low = name.lower()
    return any(part in low for part in _WAIT_INDEXERS)
...
class Prowlarr:
    def __init__(
        ...
        *,
        ...
        #: заходов на пути до трёх, а превышение цели терпится ровно одно: круг с полом
        #: в секунду стоит замеренных 2.0-4.0 с (:meth:`_circle` ждёт каждого опорного
        #: отдельно), и раздать его каждому значило бы дать молчуну не сузить каталог,
        #: а затормозить путь (:func:`torrcast.cli._no_budget`).
    ...
    def _apart(self, query: str, limit: int) -> list[RawResult] | None:
        ...
        🔴 TC-118: круг возвращается по опорным (:data:`_WAIT_INDEXERS`), а не по
        ...
        # хватало, опаздывая на 0.2 с. Ждём остаток цели (:meth:`spare`): секунды тут
        # покупают не скорость показа, а выбор между «ничего не нашлось» и картиной.
    ...
    def _circle(
        ...
        🔴 Круг кончается, когда ответили опорные (:func:`_wait_indexer`), а не последний
