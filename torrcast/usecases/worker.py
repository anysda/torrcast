...
def _cmd_worker(key: str, *, play: Callable[..., int] = _play) -> int:
    ...
    Раздача кончилась вместе с сезоном — цикл сам ищет следующий сезон
    (:func:`torrcast.usecases.next_season._next_season`), а не нашёл — говорит об этом
    строкой и гаснет чисто.
    ...
    :class:`torrcast.adapters.chromecast.cast.chromecast_receiver.ChromecastReceiver`), и он
    отвечает новому пустым статусом. Замер на живом Q70D, стык s1e5→s1e6: два соединения в ``ss``,
    «LOAD не взяли (IDLE/ERROR)», «приёмник залип — закрываю приложение и соединение», экран пустой
    **15.3 с**.
