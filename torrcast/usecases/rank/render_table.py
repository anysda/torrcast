...
def render_table(
    ...
    ``estimated`` - длительность в знаменателе битрейта это прикидка по типу картины:
    ни паспорт файла из записи состояния, ни справка её не назвали. Тогда пометки веса
    («тяжёлый», «перекодируем») посчитаны по оценке, и таблица обязана это сказать
    (TC-819): прикидка ошибается в разы, и молча отдать её за замер значило бы врать
    про каждую пометку.
