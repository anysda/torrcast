...
def gate_open(
    ...
    «Живой» здесь — доля от лидера пула (:data:`GATE_LIVENESS`): абсолютное число в
    пулах разной населённости значит разное. Знаменатель тут НЕ тот, что у
    :func:`is_full_hd`: та ступень меряет живость по сильнейшему сопернику своего
    спора, а ворота - по лидеру пула целиком, потому что вопрос у них «выбирать вообще
    не из чего», и меркой ему служит весь пул, а не одна ступень.
