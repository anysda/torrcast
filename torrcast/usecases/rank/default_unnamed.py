...
def default_unnamed(media: Media) -> bool:
    ...
    🔴 :meth:`~torrcast.stream.Media.default_track` возвращает ПОЛЕ ``index`` дорожки, а не
    её место в списке. Совпадают они ровно пока паспорт собран подряд самим ffprobe
    (:func:`torrcast.stream._track` нумерует ``enumerate`` по звуковым потокам), а паспорт,
    поднятый из кэша, приходит с теми номерами, какие в нём записаны. Поэтому место
    сверяется с длиной - тем же ходом, что и в :func:`heard`, - и промах стоит вежливого
