...
def named_elsewhere(plans: list[Plan], asked: str) -> str:
    ...
    На запросах «имя названо целиком, а дефолт встаёт на другую картину» страж называет
    обе стороны. Без ``--menu`` ступень берёт картину сама; за явным флагом показывает
    список и ждёт номера. Замер класса
