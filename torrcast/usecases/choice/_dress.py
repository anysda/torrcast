...
def _dress(menu: MenuPaint, plans: list[Plan], blocks: list[list[str]], facts: Facts) -> None:
    ...
    строку - зритель видит, как она дополняется. Ждать справку меню больше не вправе:
    ждали её полторы секунды, и в две трети прогонов она в них не укладывалась, то есть
    человек платил ожиданием и всё равно получал голый список.
    ...
    Описание тут не дописывается намеренно, и это цена решения: оно занимает не одну
    строку, а несколько, и вставить их в середину уже прочитанного списка нельзя - список
    поехал бы под курсором у человека, который его в эту секунду читает. Приехавшее
    описание попадает в кэш (:meth:`~torrcast.usecases.facts.Facts.finish`), и следующее меню
    печатает его сразу.
    """
