...
def menu_blocks(plans: list[Plan], facts: Facts | None = None, width: int = 0) -> list[list[str]]:
    ...
    Справку тут НЕ ждут: берётся то, что уже приехало
    (:meth:`~torrcast.usecases.facts.Facts.ready`), и список печатается немедленно. Не приехало
    ничего - печатается ровно та строка, что печаталась и раньше, без пустых разделителей и без
    «не нашёл».
    """
