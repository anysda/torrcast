...
def part_one_swap(plans: list[Plan], asked: str) -> str:
    ...
    Запрос «тачки» называет первую часть, а первая живая часть может оказаться другой.
    Страж сохраняет эту причину для речи: без ``--menu`` рядом называется автоматически
    взятая картина, а за явным ``--menu`` номер по-прежнему называет человек.

    Пустой ответ означает, что страж не нужен, и случаев тут три:

    * номер назван явно («тачки 2») - спрошенное уже отобрано до меню
      (:func:`~torrcast.domain.pick_franchise.pick_franchise`), и дефолт - ровно оно;
