...
def _season_reinforce(
    ...
    *,
    ...
    🔴 **Гейт против подмены.** Добор не пересобирает выдачу как попало: из ответа сезонной строки
    берутся ТОЛЬКО раздачи, у которых оригинал совпадает с оригиналом найденного сериала И имя
    которых называет нужный сезон. Без этого «Angel S01» натащил бы десяток чужих аниме («The Angel
    Next Door ... S01»): у них другой оригинал, и фильтр их отсекает. Сама картина после этого
    выбирается прежним :func:`~torrcast.domain.pick_franchise.pick_franchise`.

    Один лишний круг по индексерам, и только когда сезона в выдаче не было вовсе
    (:func:`lacks_season`): на счастливом пути добора нет. Ничего не подошло - остаётся
    прежний результат, дальше честное «раздач с сезоном N нет».

    🔴 Оригинала у вожака нет - опора только справка, и её ДОГАДКА
    (:attr:`~torrcast.domain.facts.origin.Origin.guessed`) ключом фильтра быть не вправе: имя, лишь
