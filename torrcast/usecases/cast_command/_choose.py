    ...
    with progress_bar() as progress:
        ...
        try:
            try:
                ...
                # Справка может назвать другую картину, чем та, на которую до меню
                # смотрел Enter. У выбранной тогда грелась только голова, а запасной
                # с русской дорожкой поднимался уже после её брака. Чужие прогревы выше
                # уже сняты, поэтому теперь запасной можно честно поднять для выбранной.
                bench.spare(plan, args)
            finally:
