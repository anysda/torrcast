...
class _RevivalState:
    ...
    #: С какого монотонного момента длится темнота; ``0.0`` - показ идёт. Это ЧИСЛО для
    #: арифметики выдержек, а не признак темноты: признак - :attr:`why` (часы сухого
    #: прогона считают от нуля, и нулевой момент по этому полю от картинки не отличить).
    since: float = 0.0
    ...
    ended: bool = False
    #: Место, ЗА которое уже подняли показ, перешагнув мёртвый кусок. Оно становится
    #: нижней границей следующей попытки и закладки: возвращаться в пропущенный кусок
    #: нельзя, даже если мёртвая сессия снова ответила нулём или прежним кадром.
    resume_at: float = 0.0
    #: С какого монотонного момента на конце картины ждём ВНЯТНОГО слова приёмника
    #: (:data:`WILL_LIMIT`); ``-1.0`` - не ждём, ответ был свежий.
    #:
    #: ⚠️ Отрицательным, а не нулевым: часы сухого прогона отсчитывают от нуля, и ожидание,
    #: начавшееся в нулевую их секунду, по нулю неотличимо от его отсутствия. Теми же
    #: граблями зашибался и отсчёт темноты (:attr:`why`).
    will_since: float = -1.0
