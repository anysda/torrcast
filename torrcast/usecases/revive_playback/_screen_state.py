...
class _Screen:
    ...
    #: (:meth:`torrcast.adapters.chromecast.cast.ChromecastReceiver._nudge`) прыгает вперёд по 8 с,
    #: вытаскивая приёмник из зависания, и туда же смотрит повтор LOAD посреди показа. Замерено на
    #: живом Q70D: пока экран стоял на 2:39, сторож сделал 12 прыжков и увёл позицию на 4:15 - и
    #: показ воскресал с места, которого зритель не видел, ровно на 1:36 впереди последнего кадра.
    #: Сторож при этом прав, а закладка - нет: место показа отмеряет глаз, а не указатель.
    #:
