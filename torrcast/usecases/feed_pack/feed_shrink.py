...
def _shrink(state: _State, slot: int, size: int = 0) -> bool:
    ...
    предохранитель ожидания её отпустил (:meth:`torrcast.adapters.recode.Recoder.holding`).
    ...
    (:meth:`torrcast.adapters.recode.Encode.fit`).

    🔴 Потолков ДВА, и они разной природы (TC-495). Вес куска - один; битрейт,
    который приёмник тянет (:attr:`torrcast.adapters.recode.Recoder.threshold`), - второй, и
    ...
    (:attr:`torrcast.adapters.recode.Recoder.over_wait`).
    """
