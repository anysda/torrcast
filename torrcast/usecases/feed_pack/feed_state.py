...
class _State:
    ...
    played: float = 0.0
    #: Позиция названа живым приёмником. До первого доклада вкладки сохранённое место
    #: держит упаковку, но не даёт судить, что какой-либо кусок остался позади зрителя.
    measured: bool = True
    #: Дверь показа: слот, с которого поднята упаковка. Ниже неё живого куска не будет -
    #: упаковка идёт только вперёд от захода, - поэтому плейлист такие места не обещает, а
    #: объявляет отсутствующими (:meth:`Feed.manifest`). Ноль - показ с начала фильма, и
    #: обещано всё. Пишет её единственный заход упаковки
    #: (:func:`torrcast.usecases.feed_pack.feed_restart._restart`), поэтому перемотка назад
    #: опускает дверь вместе с упаковкой и те места снова становятся обещанными.
    door: int = 0
    ...
    shelf: float = 0.0  # отметка полки прогрева при последнем осмотре (:func:`_warming`)
