...
class _State:
    ...
    #: (:func:`torrcast.usecases.playback._warmer._warmer`). Прогретый кусок и живой - это одна
    #: лента для приёмника, и если они закодированы по-разному, на стыке источников у него меняется
    #: SPS.
    ...
    #: каждом опросе.
    slack: float = 0.0
    #: Кодировщик живых кусков (:class:`torrcast.adapters.recode.recoder.Recoder`) или ``None``.
    #: Пока у него идёт заход, прогрев замирает: см. :meth:`_must_yield`.
    ...
    #: Сколько заходов подряд это место не дало НИ ОДНОГО куска (:func:`_barren`). Ключ -
    #: слот. Считается врозь от :attr:`skews`: там кусок лёг и лёг не туда, а тут его нет
    #: вовсе, и сверять нечего.
    barren: dict[int, int] = field(default_factory=dict)
    #: Места, которые упаковка не берёт (:data:`BARREN_TRIES` пустых заходов подряд). Целью
    #: прогрева они быть перестают (:func:`_missing`), иначе прогрев топчется на первом же
    #: таком месте вечно и точечный перекод тяжёлых мест, идущий ПОСЛЕ укладки, не
    #: начинается вовсе.
    ...
    misgrid: int = -1
    #: Сколько уложенных кусков сверка НЕ СМОГЛА сверить с сеткой
    #: (:data:`torrcast.usecases.warm.verify.BLIND`). Считается отдельно от :attr:`skews`
    #: намеренно: промах - приговор куску, а это признание сторожа, и путать их значит
    #: держать зелень там, где мерить не могли (TC-879). Заход по этому числу не
    #: обрывается: слепота сторожа - не поломка укладки.
    unchecked: int = 0
