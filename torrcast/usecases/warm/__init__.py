...
* **прогрев не имеет права мешать показу.** Он идёт ``nice`` и в темпе, который
  ...
  проседает — встаёт (``SIGSTOP``) и ждёт. Приоритет всегда у того места, где смотрят
  прямо сейчас.
...
кодировщика идёт заход (:attr:`torrcast.adapters.recode.recoder.Recoder.working`), прогрев
стоит. С ``SIGSTOP`` тот же перекод идёт 2.62× — вровень с пустой машиной.
