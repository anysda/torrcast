...
class _Clock(NamedTuple):
    ...
    began: float
    #: Метка куска - это время фильма (``True``, mpegts) или мерить его тут нечем
...
def segment_start(path: Path) -> _Clock:
    ...
    можно прямо. У CMAF - нельзя, и ``nan`` тут не «файл битый», а «этой мерой тут не
    меряют». Пока ответом было одно число, два случая были неотличимы: ``nan`` на любом
    ``.m4s`` сторож прогрева читал как «годен», а показ - как «мимо сетки» и СТИРАЛ
    прогретое, то есть прогретое на приставке не доезжало до зрителя вовсе (TC-879).

    🔴 Фрагмент CMAF тут намеренно НЕ разбирается, хотя разбирается легко. Счётчик ``tfdt``
    считает прогон муксера, а не фильм. Замер настоящим ffmpeg (ровная сетка по 4 с, копия
    видео, два захода на одну сетку): слот 16-20 с фильма несёт 16.000 из захода с нуля и
    4.000 из захода с 12-й секунды - один кусок фильма, два прогона, два разных числа. Ни
    один потребитель это число сверить не может, оба ветвятся по :attr:`_Clock.movie`
    раньше, чем посмотрят на :attr:`_Clock.began`, а разбор стоит горячего пути: 73.4 мкс
    против 32.2 мкс медианой на настоящем ``.m4s``, то есть вдвое с лишним за число,
    которого никто не читает. Вернуть фрагменту время фильма нечем: ``-copyts``,
    ``-output_ts_offset`` и ``-avoid_negative_ts disabled`` до него не доезжают, это же
    измерено с другой стороны у склейки на ленту показа
    (:func:`torrcast.adapters.stream_pack.splice_on_tape.splice_on_tape`). Сверять счётчик
    прогона с сеткой имеет право только тот, кто эту ленту ИЗМЕРИЛ по своему же опорному
    куску (:func:`torrcast.adapters.stream_pack.run_tape.run_tape`), а у сверки укладки
    опорного куска нет.

    ``nan`` - честное «не знаю»: файл не читается, не выровнен по пакетам TS, меток в
    голове нет вовсе либо это фрагмент CMAF, где времени фильма нет ни в одном байте.
    Гадать тут нельзя ни в одну сторону: сторож на догадке выбрасывал бы здоровые куски.
    """
    try:
    ...
    if head[4:8] in _MP4_HEADS:
        return _Clock(math.nan, movie=False)
