...
Зовёт его сборка сетки показа (:func:`layout`), и только она.
"""
...
def _encode_all(
    ...
    вниз, и потолок едет в :attr:`torrcast.adapters.recode.encode.Encode.ceiling` вместе с самим
    кадром: решение «во что ужимать» принимается здесь, один раз, до первого сегмента.
