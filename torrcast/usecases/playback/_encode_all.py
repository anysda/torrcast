...
def _encode_all(
    ...
    умолчанию она выключена, потому что цвет тут покупается скоростью. Замер на 4К-исходнике
    («Матрица» 2160p HDR10, ``ultrafast`` 9 Мбит/с, тяжёлое место): со скейлом до 1080p
    перекод идёт 1.34-1.41x реального времени, а с тонемапом после того же скейла - 1.00x,
    то есть упаковка идёт вровень с показом и запаса от подгрузов не остаётся. 🔴 Числа
    сняты на одном стенде в 4 vCPU и на другой машине будут другими; неизменно тут одно -
    тонемап забирает у 4К-пути весь запас скорости, каким бы тот ни был.
