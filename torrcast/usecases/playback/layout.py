...
def layout(
    ...
    grid = _state.grid_for(
        ...
        # Задаём его МГНОВЕННЫМ потолком кодера
        # (:attr:`torrcast.adapters.recode.encode.Encode.maxrate`), а не целью: цель - это средний
        # битрейт по прогону, а в отдельный кусок кодер вправе положить вплоть до потолка
        # (:data:`torrcast.adapters.recode.encode_settings.MAXRATE_GAIN`), и на трудном материале он
        # ровно это и делает. Замер на стенде (1080p10 40 Мбит/с, ultrafast, цель 9): насыщенный
