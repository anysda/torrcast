...
def _asked_kind(lead: Picture | None, args: Args) -> bool | None:
    ...
    * сам запрос - серию называет человек, и «клиника s1e1» это утверждение о сериале,
    ...
    лишь согласию» (:func:`~torrcast.usecases.passport_either.PassportEither.of`). Согласия у сериала
    с фильмом не бывает ровно тогда, когда одноимённый фильм существует: «Дедвуд» и «Клиника» на
    живой пробе так и теряют имя (``Deadwood``, ``Scrubs``) - а без имени второй заход уходит
    транслитом в никуда. Молчание о серии по-прежнему НЕ значит «фильм»: сериал зовут и без номера,
    поэтому там остаётся прежний осторожный режим.
    """
    if lead is not None:
        return lead.kind == "tv"
