...
def _second_language(
    ...
    *,
    ...
    ⚠️ Право у гейта ровно одно - НЕ ДОБАВИТЬ своё. Из найденного по-русски уходят лишь
    соседи по слову (:func:`_neighbours_only`), остальное отказ от добора возвращает
    целиком (:func:`_as_is`), а не пустотой.
