...
def _second_language(
    ...
    *,
    ...
    тощем пуле: на полной выдаче (порог :data:`~torrcast.domain._name_data.THIN_POOL`) поиск остаётся
    ...
    круга - целая цель (:attr:`~torrcast.adapters.prowlarr.prowlarr.Prowlarr.cap_floor`), а съеденный
    остаток не отменяет заход, а объявляется строкой.
