...
class _BenchTrouble(_BenchWork):
    ...
    def _trouble(
        ...
        файла (:meth:`torrcast.domain.media.Media.weight_mbit`). Отбраковка спрашивает
        ...
        (:attr:`torrcast.domain.profile.Profile.recode_codecs`), и аниме — жанр, где HEVC бывает
        вообще всем, что нашлось, — теперь играет. Предпочтение H.264 при прочих равных живёт не
        здесь, а в ранжире (:func:`rank_releases` топит hevc ниже всех), то есть сплошной перекод
        ...
        (:attr:`torrcast.domain.config.Config.bitrate_hard_mbit`), и кадру выше 1080p он достаётся
