...
class Bench(_BenchPrewarm):
    ...
    def resolve(self, plan: Plan, args: Args, progress: Progress) -> _Prep:
        ...
        * **молчание роя** — метаданные не приехали, поток не прочитался, фаза не
        ...
        ДОРОЖКОЙ НА ЯЗЫКЕ ЗРИТЕЛЯ» - русской под русской ручкой, английской под
        английской (:func:`sought_voice`), - и годен только ПОДТВЕРЖДЁННЫЙ язык
        (:func:`voice_unproven`). Считается она как приговор - про релиз узнали всё, - и
        упирается в те же два потолка. Лишнего времени это не стоит: спрашивается уже
        прочитанный паспорт.
        ...
        очередь кончилась и искомой дорожки не нашлось ни у кого (:meth:`_mute_fallback`):
        тогда зритель слышит про японский звук и решает сам. Паспорт, промолчавший про
        язык, запасным ходом не становится вовсе (:meth:`_Tally.hold`), а обход, срезанный
        ...
        for attempt, number in enumerate(queue, start=1):
            ...
            # у которого нет дорожки на языке зрителя, годным не считается, и очередь идёт
            # дальше. Спрашивается уже прочитанный паспорт (:func:`voice_unproven`) - ни
            # одного лишнего ffprobe и ни одного лишнего похода в рой это не стоит.
            # Названный руками релиз не судим: там человек выбрал сам.
            #
