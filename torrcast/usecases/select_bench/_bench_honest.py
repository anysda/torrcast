...
class _BenchHonest(_BenchTrouble):
    ...
    def _honest(
        ...
        только релиз с ПОДТВЕРЖДЁННОЙ дорожкой на языке зрителя (:func:`voice_unproven`),
        а безымянный паспорт теперь бракуется гейтом и очередь идёт дальше сама. Спросить
        соседа, обещавшего озвучку именем, отдельной машинкой больше незачем: он и так
        ...
        for number in rest:
            ...
            if why:
                ...
                continue
            # 🔴 TC-178. Честный 1080p без дорожки на языке зрителя - это не «лучше»:
            # разрешение выиграно, а картина осталась несмотренной. Взятый сюда доходит
