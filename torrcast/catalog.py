...
def reads_season(pictures: list[Picture]) -> bool:
    ...
    * ни одну картину каталог не подписал номером части. Подписал («Форсаж 5», «Пила 8»,
      ...
      Исключение из второго условия - номер, за который поручились сами раздачи, назвав
      его СЕЗОНОМ (:func:`_numbered_season`).
...
def _numbered_season(picture: Picture) -> bool:
    """Номер, прилипший к имени картины, - её СЕЗОН, и поручились за это её же раздачи.

    🔴 TC-564. Условие «ни одну картину каталог не подписал номером части» закрывалось
    само на себе. Когда ВСЕ раздачи сериала подписаны одним сезоном, имя каждой начинается
    числом («Кухня 6 / Kuhnya 6 (2017) ... 6 сезон»), число прилипает к названию картины,
    и каталог заводит её с ``part=6``. Дальше номер запроса читался номером части, план
    просил ``s1e1``, накрыть его не могла ни одна раздача (:meth:`Release.covers`) - и
    `cast кухня 6` отказывал совсем: «раздач 2, но сезона 1 среди них нет - названы 6».
    Показа не было вовсе при живой картине и живых раздачах.

    Признак спрашивается у самих раздач, а не у догадки, и совпасть должно всё сразу:

    * картина - СЕРИАЛ. У фильма сезонов не бывает, и «Форсаж 5» сюда не попадает ничем;
    * раздачи назвали сезоны САМИ (:func:`~torrcast.franchise.seasons_named`) - молчание
      именем в счёт не идёт, оно значит «может быть», а не «шестой»;
    * назван ровно один сезон, и он же и есть прилипшее к имени число. Совпадение числа
      с сезоном - это и есть поручительство: раздача, назвавшаяся «6 сезон» под именем
      «Кухня 6», говорит про свою шестёрку то же самое, что человек про свою.

    ⚠️ Подмены части тут не появляется: играет ровно та картина, которую человек назвал,
    и меняется только то, какой сезон у неё спрошен. Соседняя часть франшизы вместо
    просимой по-прежнему не подставляется - за это отвечает :func:`_numbered`, и правило
    это признак не трогает.

    ⚠️ Строгое равенство - ограждение, а не придирка. Назвали раздачи два сезона и больше
    («1-6», «5-10») - число в имени за сезон уже не ручается: сезонов много, а число одно,
    и таким пулом первый сезон обычно накрыт, то есть чинить нечего.
    """
    return (
        picture.kind == "tv"
