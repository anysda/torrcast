"""Консоль: ввод, который работает из коробки, и живой прогресс по фазам (§3, §4 SPEC-v2).

Три претензии владельца сходятся в этом модуле:
...
Отдельное правило вывода (§3): ни ``→``, ни ``⚠``, ни ``▶``, ни ``≥`` наружу не уходит —
в терминалах владельца они и не несут смысла, и ломают ширину. Слова несут.
"""
...
def clean(text: str) -> str:
    ...
    Битый ввод не должен доезжать до парсера ни при каких обстоятельствах (§3): pty без
...
def iutf8() -> int:
    ...
    платформах, а нам он нужен ровно на Linux, где владелец и работает.
    """
    import termios
...
def terminal() -> Iterator[None]:
    """Включить ``IUTF8`` на stdin и вернуть режим как было (§3).

    Без него ssh-сессия владельца ведёт себя так: русская буква занимает два байта, а
...
def ask_line(question: str, default: str = "") -> str:
    ...
    сценарий: спросить всё равно некого (§3).
    """
    prompt = f"{question}: "
    if not stdin_is_tty():
...
def ask(question: str, count: int, default: int = 1) -> int:
    """Вопрос с номерами: принимает и цифру, и пустой Enter (§2 SPEC-v2)."""
    while True:
        answer = ask_line(f"{question} [{default}]")
        if not answer:
            return default
...
class Progress:
    """Живой прогресс по фазам с бегущим временем (§4 SPEC-v2).
    ...
    ли: молчание дольше пары секунд — это и была претензия №4 владельца.
    ...
    def _close_line(self) -> None:
        """Закрыть строку фазы её итоговым временем — оно и есть замер (§4)."""
        if not self._text:
            return
