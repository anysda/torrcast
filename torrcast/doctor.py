...
# Старое имя остаётся только как уже загруженный модуль, без плоского файла-сироты.
sys.modules.setdefault("torrcast.doctor_checks", sys.modules[__name__])
...
def _cache(config: Config) -> Line:
    ...
    Кэш - это запас показа на обрыв интернета, поэтому строка тут не про красоту: замер
    ...
    * кэш НА ДИСКЕ платит МЕСТОМ, а память берёт себе постоянную сотню мегабайт
    ...
    if sets.get("UseDisk"):
        return _cache_on_disk(size, str(sets.get("TorrentsSavePath") or ""))
    total = machine_memory()
    weight = size * CACHE_OVERHEAD
    text = (
        f"кэш раздачи {_gib(size)} в памяти, под показом это ~{_gib(weight)} памяти "
...
def _cache_on_disk(size: int, path: str) -> Line:
    """Кэш на диске: память ему не мера, мера - место рядом с прогревом."""
    text = f"кэш раздачи {_gib(size)} на диске ({path or 'путь не задан'})"
    if not path:
        return _bad(f"{text} - служба положит его куда сама решит, переставь install.sh")
    free = disk_free(path)
    if not free:
        return _warn(f"{text}, свободное место на разделе не читается")
    text = f"{text}, память службы ~{_gib(CACHE_ON_DISK_MEMORY)}, на разделе {_gib(free)}"
    if free < CACHE_DISK_RESERVE:
        return _bad(f"{text} - прогреву места не остаётся, обрыв оборвёт показ")
    return _ok(text)
