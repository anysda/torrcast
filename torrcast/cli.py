...
#: доходит вовсе.
#:
#: Шесть секунд - это втрое дольше, чем службе нужно на первый контакт после добавления
...
def silent_swarm(plan: _Plan, touched: int, shown: str) -> str:
    ...
    потрогали, не говорит ни слова о тех, кого не трогали. Поэтому строк три, и
    ...
    * потрогали часть - про остальных мы не знаем ничего, и врать за них нельзя.
      Человеку остаётся ручной выбор, и строка говорит, чем именно он располагает.
    ...
    которая молчит поток, показывает в выдаче сотню сидов ровно так же бодро.

    🔴 Ход человеку предлагает КАЖДАЯ из трёх, и ходы эти разные, потому что разное
    осталось непроверенным. Там, где отбор до части выдачи не дошёл, ход - ручной выбор:
    непроверенные раздачи есть, и они рядом. Там, где потрогали всё, ручной выбор врал бы
    надеждой - выбирать не из чего, и честный ход другой: назвать картину иначе (другой
    запрос собирает другую выдачу) или вернуться позже, когда рой проснётся. Отказ без
    хода - это тупик, в котором человек остаётся один; раньше таким был отказ по
    :data:`PEER_GRACE`, а он теперь и самый частый.
    """
    total = len(plan.ranked)
    peers = max((release.seeders for release in plan.ranked), default=0)
    ...
    return (
        f"{counts} - эти молчат, до остальных отбор не дошёл (в выдаче числится "
        f"до {peers} сид): выбери руками - cast releases <запрос>, потом "
        f"cast <запрос> --release N ({shown})"
    )
...
class _Bench:
    ...
    def spare(self, plan: _Plan, args: Args) -> list[_Prep]:
        ...
        return [self.start(plan, number) for number in queue[1 : 1 + PREWARM_SPARE]]
    ...
    def resolve(self, plan: _Plan, args: Args, progress: Progress) -> _Prep:
        ...
        * **молчание роя** — метаданные не приехали, поток не прочитался, фаза не
        ...
        if verdicts == 0 and exhausted and tried:
            # «Не нашли» и «рой промолчал» - разные отказы. Очередь пройдена до конца, и ни
            # один релиз не дошёл до приговора: ffprobe не прочитал ни одного, потому что не
            # приехали ни метаданные по DHT, ни поток. Раздачи есть и по именам годны - молчит
            # рой, а не выбор. Врать «годного релиза нет» тут нельзя. Но и «рой мёртв» на всю
            # выдачу - враньё ровно так же: очередь отбора это НЕ вся выдача, и сколько из неё
            # потрогали, знают только счётчики - они и печатаются (:func:`silent_swarm`).
            raise NotFoundError(silent_swarm(plan, len(tried), f"{shown}{more}") + tail)
        raise NotFoundError(
    ...
    def _trouble(
        ...
        call = self.profile.verdict(prep.media.video or "", prep.media.depth)
...
def _encode_all(
    ...
    ⚠️ Успеет ли сплошной перекод, здесь не спрашивается: кадр выше
    :data:`torrcast.recode.RECODE_HEIGHT` отбраковывается раньше и честно, на отборе
    (:meth:`_Bench._trouble`). Сюда такой файл доходит только названным руками.
...
class _Revival:
    ...
    * источник, которого мы СПРОСИЛИ (:class:`torrcast.stream.Supply`), снова отвечает и
    ...
    куски: :data:`REVIVE_PAUSE` до первого LOAD, как и между остальными.
    ...
    def resurrect(self, receiver: Receiver, feed: Feed, warmer: Warmer | None, pos: float) -> bool:
        ...
        if self.dropped and dark < self.pause:
            ...
            # Замерено на живом Q70D: попытка 1 при «темноте 0 с» ушла в ресивер, который
            # после 404 берёт LOAD как ни в чём не бывало (наказание - ноль секунд,
            # замер 09-08-2026), но экран занят его же приложением ещё 301 с.
...
def hevc_hope(release: Release, last: bool) -> bool:
    ...
    Кадр выше 1080p сюда не проходит и последней надеждой
    (:data:`torrcast.recode.RECODE_HEIGHT`).
    ...
    ⚠️ Потолок кадра — НЕ поле профиля приёмника, и переносить его туда не надо: он про
    наши 4 vCPU, а не про телевизор, и живёт он там же, где пресет и множитель битрейта
    (:data:`torrcast.recode.RECODE_HEIGHT`). Ступень лестницы качества
    (:data:`FULL_HEIGHT`) носит то же число по другому поводу: она про РЕЛИЗ.
    Три источника ограничений — приёмник, наша машина и релиз — тут сходятся в одной
    строке, и путать их нельзя.
    """
    return last and release.is_hevc and release.height <= RECODE_HEIGHT
...
def pack_mbit(release: Release, runtime: float) -> float:
    ...
    в очереди перебора ВЫШЕ живого сериала на 61 сид. Три подряд таких верха — это три
    приговора ffprobe (``mpeg4``), то есть весь :data:`MAX_TRIES` и 130 секунд, после
    которых показ говорит «годного релиза нет» при живом каталоге.
