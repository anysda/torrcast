...
#: ближайшая по очереди (:meth:`_Bench.spare`). 🔴 TC-309. Проверка честности спрашивает
#: такого соседа первым же вопросом, когда взятый релиз про язык звука не сказал ничего
#: (:meth:`_Bench._honest`, повод «язык звука не назван»), а стоит он в очереди где
#: угодно - на 4-9 месте, - и с нуля в :data:`HONEST_BUDGET` укладывался не всегда:
#: подъём это метаданные роя (3-6 с) плюс чтение дорожек. Греется он ровно тогда, когда
#: этот вопрос вообще может прозвучать: молчаливое имя есть у кого-то из первых
#: :data:`MAX_TRIES` кандидатов, а сосед русскую обещает. Релиз, обещавший русскую сам,
#: соседа про звук не спрашивает никогда (:func:`unnamed_sound`) - и картина, у которой
#: обещали все первые кандидаты, лишней раздачи в рое не получает.
PREWARM_DUB = 1
...
def unfit_line(plan: _Plan, drops: dict[str, int]) -> str:
    ...
    Поэтому ход строка предлагает один - живых соседей по франшизе (:func:`kin_line`),
    - а «выбери руками» не предлагает: раздачи отвергнуты по уже ИЗВЕСТНЫМ признакам, и
    номер этого не меняет. Названный человеком релиз при этом играет как играл: ворота
    на него не смотрят вовсе (:meth:`_Plan.candidates`).
...
class _Bench:
    ...
    def spare(self, plan: _Plan, args: Args) -> list[_Prep]:
        ...
        может оказаться «дорожкой без тега», и тогда проверка честности первым же вопросом
        спрашивает ближайшего обещавшего русскую соседа (:meth:`_honest`, повод «язык
        звука не назван»). Стоит тот в очереди где угодно - на 4-9 месте, - и с нуля в
        :data:`HONEST_BUDGET` укладывался не всегда: замер по живой выдаче - подъём
        метаданных 3-6 с плюс чтение дорожек против двенадцати секунд на всё, общих на все
        вопросы. Здесь он греется заодно с обычным запасным, и первому вопросу достаётся
        уже готовый ответ - а бюджет остаётся целым для следующего.

        Греется ровно тогда, когда вопрос вообще может прозвучать. Спрашивается не верх,
        а тот, кого отбор реально возьмёт, поэтому молчаливое имя ищется среди первых
        :data:`MAX_TRIES` кандидатов - верх и два ближайших запасных. Релиз, обещавший
        русскую своим именем, соседа про звук не спрашивает никогда (:func:`unnamed_sound`);
        а картина, у которой обещали все трое, лишней раздачи в рое не получает вовсе.
        """
        if args.release is not None:
            return []
    ...
    def resolve(self, plan: _Plan, args: Args, progress: Progress) -> _Prep:
        ...
        * **молчание роя** — метаданные не приехали, поток не прочитался, фаза не
        ...
        if not queue:
            ...
            # картины, худший вид брака. Отказ честный: сколько раздач было и почему
            # каждая не годится (:func:`unfit_line`), плюс живые соседи по франшизе.
            offer = kin_line(plan.kin)
            raise NotFoundError(unfit_line(plan, drops) + (f"\n{offer}" if offer else ""))
    ...
    def _honest(
        ...
        двух ffprobe, и не дольше :data:`HONEST_BUDGET`. Сосед по ЗВУКУ стоит в очереди
        дальше запасного, и его под меню греет свой прогрев (:data:`PREWARM_DUB`,
        :meth:`spare`): холодный он в бюджет влезал не всегда - подъём с нуля это
        метаданные роя (3-6 с) плюс чтение дорожек против двенадцати секунд на всё.
        ...
        🔴 TC-301. Поводов заглянуть к соседу теперь ДВА, и машинка на них одна.

        Второй повод - ЯЗЫК ЗВУКА (:func:`unnamed_sound`): взятый релиз про него не сказал
        ничего - ни паспортом (дорожка без тега), ни именем, - а ниже в очереди стоит
        нетронутая раздача, чьё имя русскую дорожку прямо обещает. Гейт такой релиз
        пропускает по делу (бракуя его, мы бракуем не релиз, а собственное незнание), и
        ровно поэтому очередь до обещанной русской не доходит вовсе: показ уже начался.

        Разница между поводами ровно в двух местах - кого спрашивать и что считать
        победой. Для разрешения это «обещает больше кадра» и «внутри правда больше»
        (:func:`promises_more`), для звука - «обещает русскую именем» и «русская правда
        есть в паспорте» (:attr:`~torrcast.stream.Media.russian`). Всё остальное - бюджет,
        число вопросов, уборка спрошенного, строка на каждое решение - общее, и нового
        тут не появляется ничего.

        Стоит это столько же, сколько стоила проверка разрешения: не больше
        :data:`HONEST_BUDGET` на всё и не больше :data:`MAX_TRIES` вопросов, а не успевший
        ответить сосед отпускается со своей строкой. Подгрузов повод не добавляет: сосед
        проходит тот же :meth:`_trouble` с теми же потолками, и тяжёлый до показа не
        доедет.
        """
        ...
        rest = [
            ...
            and n not in judged
            and (
                plan.ranked[n - 1].dubbed
                if deaf
                else promises_more(plan.ranked[n - 1], chosen.found)
            )
        ][:MAX_TRIES]
        ...
        for number in rest:
            ...
            if why:
                ...
                continue
            if deaf:
                # 🔴 TC-301. Победа тут одна - ПОДТВЕРЖДЁННАЯ русская дорожка. Обещания
                # имени хватило, чтобы соседа спросить, но не хватит, чтобы ради него
                # менять релиз: имя врёт в обе стороны, и меняем мы незнание на знание,
                # а не одно обещание на другое.
                #
                # Кадр в этом размене не участвует, и это то же решение, что уже принято
                # в ранжире (:func:`sound_step` стоит ВЫШЕ :func:`is_full_hd`): «1080p
                # вместо 720p» - оттенок, а «по-русски вместо неизвестно как» - разница
                # между «посмотрел» и «не посмотрел». Молчаливым размен при этом не
                # становится: ступень ниже доступной называет :func:`stepdown_note` в
                # строках перед стартом (:meth:`_announce`).
                if not alt.found.russian:
                    _turned_down(judged, number, "русской дорожки нет")
                    print(f"релиз {number} - русской дорожки в нём тоже нет")
                    self._forget(alt)
                    continue
                print(
                    f"язык звука в релизе {chosen.number} не назван - "
                    f"беру {number}, там русская дорожка"
                )
            ...
            return alt
        if deaf:
            print(f"язык звука в релизе {chosen.number} не назван - русской рядом нет, играю его")
            return chosen
...
def unnamed_sound(release: Release, media: Media, *, native: bool = False) -> bool:
    ...
    (:meth:`_Bench.resolve`). А между ними лежит третий, и до сих пор он молча засчитывался
    за первый: дорожка БЕЗ ТЕГА ЯЗЫКА. Гейту достаточно одной такой, чтобы пропустить
    релиз, - и это правильно, выкидывать картину по догадке нельзя. Неправильно другое:
    ниже в той же очереди стоит раздача, чьё ИМЯ русскую дорожку прямо обещает, и её никто
    не спрашивает. Человек в итоге слышит неизвестно что там, где рядом лежало известно
    что.

    Имя взятого релиза спрашивается вторым условием и снимает повод: раздача, обещавшая
    русскую именем (:attr:`~torrcast.parse.Release.dubbed`), - это уже не незнание, а
    основание, и подглядывать за соседом ради него незачем.

    ⚠️ Что такое дорожка без тега на самом деле, эта строка не решает и решать не должна -
