...
def alive_numbers(plans: list[_Plan], numbers: list[int]) -> list[int]:
    """Номера картин, прошедших порог живости; пусто - живых нет вовсе.

    Порог - доля от самой живой картины (:data:`FULL_HD_LIVENESS`), см. :func:`first_alive`.
    """
    best = max((liveliness(plans[n - 1]) for n in numbers), default=0)
    if best <= 0:
        return []
    floor = best * FULL_HD_LIVENESS
    return [n for n in numbers if liveliness(plans[n - 1]) >= floor]
...
def default_note(plans: list[_Plan]) -> str:
    """Чем дефолт обязан не хронологии; пусто - решения не принимали, и говорить нечего.

    Молчаливых подмен нет: и гейт типа (:func:`asked_kind`), и пропуск однораздачной
    картины (:func:`backed`) уводят дефолт с той картины, на которую он сел бы сам, - и
    человек слышит, с какой и на какую. Строка печатается только тогда, когда дефолт от
    решения ДЕЙСТВИТЕЛЬНО сдвинулся: сказать «беру среди сериалов» там, где сериал и так
    оказался бы первым, значит говорить о решении, которого не принимали.
    """
    ...
    if len(numbers) != len(plans) and picked != plain:
        return (
            f"спросили серию - дефолт беру среди сериалов: "
            f"«{_named(plans[picked - 1].picture)}», а не «{_named(plans[plain - 1].picture)}»"
        )
    alive = alive_numbers(plans, numbers)
    if alive and alive[0] != picked:
        return (
            f"у «{_named(plans[alive[0] - 1].picture)}» всего одна раздача - дефолт беру "
            f"на «{_named(plans[picked - 1].picture)}», там раздач {len(plans[picked - 1].ranked)}"
        )
    return ""
