...
TABLE_LIMIT = 12
#: Сколько релизов подряд проверяем ffprobe, прежде чем сдаться: подмены не молчат.
MAX_TRIES = 3
...
def bitrate_of(release: Release, duration: float) -> float:
    ...
    самые обсиженные раздачи сезона улетали бы вниз с пометкой. Сериалу битрейт меряется
    по файлу серии, когда он открыт и прочитан ffprobe.
    """
