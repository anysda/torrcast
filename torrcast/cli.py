...
def _second_language(
    ...
    * франшиза - когда года не назвал никто.
    ...
    (:attr:`~torrcast.facts.Origin.guessed`), проверенным не считается:
    ...
    * своего русского имени у статьи нет (аниме подписано латиницей) - сверить было не с
    ...
    # 🔴 TC-253. Русская выдача не дала картины вовсе - сверять добор будет НЕ С ЧЕМ, и
    # гейт (:func:`same_picture`) верит второму имени на слово. А справка на этом пути
    # ошибается уверенно: статьи «Все мы незнакомцы» в Википедии нет, и по сходству имён
    # находится «Все мы убийцы» - французская картина 1952 года. Раньше эту ложь чаще
    # прятал потолок в полторы секунды; теперь справке тут отдают весь остаток цели, и
    # доезжать она будет ЧАЩЕ. Поэтому имя, лишь признанное похожим, само себя не
    # доказывает - ему нужен второй признак.
    hearsay = lead is None and about.guessed and slugify(alt) == slugify(about.title)
