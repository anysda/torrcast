...
def level_for(frame: int) -> str:
    ...
    Ступень ноль - кадра не спрашивали (паспорт прежней версии): отдаём прежние ``4.1``,
    потому что и кадр в таком прогоне остаётся нетронутым, каким был до этой правки.
    """
    if frame <= 0:
        return _LEVELS[0][1]
