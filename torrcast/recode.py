...
class Weights:
    ...
    def size(self, slot: int, span: float) -> float:
        ...
        Точность замерена 08-08-2026 на «Тачках 3»: предсказание 57.8 МБ против 51.4 МБ
        уехавших на самом деле (+12 %, в безопасную сторону). То есть картой предсказывать
        вес копии МОЖНО — промах §6.2.5 был не в карте, а в потолке перекодирования.
        """
    ...
    def bulky(self, grid: Grid, cap: float) -> tuple[int, ...]:
        ...
        Отдельно от :meth:`heavy` и не совпадает с ним: замер 08-08-2026 по картам трёх
        релизов при пороге 10 Мбит/с — «Тачки 3» 5 таких кусков вне тяжёлых, «Моана 2» 3,
        «Моана» 2016 (лёгкое кино, тяжёлых почти нет) 2 куска по 17–18.3 МБ. Каждый из
        них — это ровно тот стоп 4–8 с, что разобран в §6.2.4, только пришедший не
        битрейтом, а длиной куска на лёгком месте.
        """
...
class Recoder:
    ...
    def targets(self) -> tuple[int, ...]:
        ...
        Две мерки, и они не совпадают (§6.2.6). Первая — битрейт: приёмник не тянет
        ...
        (§6.2.4), а такой кусок бывает и на скромных 12 Мбит/с, если он длинный. Замер по
    ...
    def oversize(self, slot: int, size: int = 0) -> bool:
        """Копия этого куска тяжелее потолка, то есть наружу её отдавать нельзя (§6.2.6).
    ...
    def holding(self, slot: int, size: int = 0) -> bool:
        ...
        ⚠️ **Одно исключение стоит выше всех сроков: копия тяжелее потолка** (§6.2.6).
        Срок тут ни при чём — такой кусок не «хуже», он гарантированно валит приёмник
        (§6.2.4, замер: 19.4 МБ — стоп 8 с, 24 МБ — потеря сессии целиком). Поэтому
    ...
    def _hold_bulky(self, slot: int, now: float) -> bool:
        ...
        Замер, ради которого правило написано (§7.9, смок 28; «Тачки 3» со старта 3880):
        ...
        Ждём, пока кодировщик за кусок берётся. Три случая, когда ждать нечего и копия
        уходит как есть (худшее из возможного — сегодняшнее поведение, а не чёрный экран):
        кодировщик снят, кусок не в его списке или он на нём уже сдался (:attr:`done`).
        Плюс предохранитель :attr:`over_wait` — на случай, когда ffmpeg не поднимается вовсе.
        """
        from torrcast.stream import MAX_SEGMENT_BYTES
        ...
        if since is None:
            ...
            self._say(
                ...
                f"такой кусок приёмник не доигрывает (§6.2.4)"
            )
        if now - since < self.over_wait:
            self.blocked = slot
            return True
        # Предохранитель сработал: кодировщик за эти секунды не дал ничего. Отдаём копию
        # и говорим об этом прямо — это единственный оставшийся путь к ребуферу.
        self._say(
            f"⚠️ v{slot}: перекода нет {self.over_wait:.0f} с — отдаю тяжёлую копию, "
            f"приёмник может встать"
        )
        self._unstick(slot)
        return False
    ...
    def note(self, slot: int, how: str) -> None:
        ...
        копии, §6.2.5) или ``перекод`` (склейка не вышла, §6.2.8). Копия — это
        ...
        # Отказ склейки — это вернувшийся разрыв на голове захода (§6.2.8), и молчать о
