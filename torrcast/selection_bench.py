...
class _Bench:
    ...
    def resolve(self, plan: _Plan, args: Args, progress: Progress) -> _Prep:
        ...
        * **молчание роя** — метаданные не приехали, поток не прочитался, фаза не
        ...
        # Первые приговоры независимы друг от друга: паспорт третьего релиза не должен
        # ждать, пока ffprobe закончит первые два. Порядок решений остаётся очередным —
        # здесь только запускается работа, а читает ответы цикл ниже. Три — тот же
        # предел приговоров, поэтому лишнего перебора выдачи и новых авто-решений нет.
        for number in queue[:MAX_TRIES]:
