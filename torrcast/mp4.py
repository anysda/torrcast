...
сверх. Замер 06-08-2026 на стенде, «Moana.2.2024.REPACK.1080p.BluRay.x264.AAC5.1-[YTS.MX]»
...
def _boxes(window: _Window, start: int, end: int) -> Iterator[tuple[bytes, int, int]]:
    ...
    06-08-2026 на стенде, «Моана 2» от YTS).
    """
    at = start
    while at + 8 <= end:
...
def _sample_times(window: _Window, stbl: tuple[int, int], wanted: list[int]) -> list[int]:
    ...
    чистого процессора (§7.2 SPEC-v2) — повторять эту цену незачем.
    """
