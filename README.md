...
показа: «играю ... с 0:41:03». `--new` играет ту же сохранённую раздачу, тот же файл и
ту же дорожку с начала.
...
Всё разом, вместе с раскладкой и ffmpeg-стадией, гоняет `scripts/test-gate`. Куда класть
новый файл и откуда его можно звать - `docs/layout.md`: код разложен по слоям
(`domain` / `ports` / `usecases` / `adapters` / `cli` / `runtime`), направления импортов
между ними считает `scripts/structure-gate`, а карту публичных символов -
`scripts/symbolmap.py` в `docs/map.md`.
