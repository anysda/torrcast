...
- Не меньше ~8 ГиБ RAM: живое окно сегментов HLS живёт в `/dev/shm`, рядом работают два
  ffmpeg. Кэш раздачи - запас показа на обрыв интернета - за эту память больше не
  борется: установка кладёт его туда, где его помещается больше, а это обычно диск
  (на машине с большой памятью и тесным диском - память). Размер считается сам, ручное
  переопределение - `TORRCAST_TS_CACHE`.
- ~30 ГБ свободного диска: ~20 ГБ под прогрев (`/var/lib/torrcast/warm`, бюджет меняется
  в конфиге) и до 8 ГиБ под кэш раздачи (`/opt/torrcast/torrserver/cache`, стирается
  сам). Худший случай прогрева - три часа на потолке 9 Мбит/с, это ~13 ГБ; бюджет на
  всех один, и новый фильм вытесняет самый давний. Места мало - прогрев честно скажет
  об этом и встанет, показ продолжится как раньше, окном, а кэш установка урежет или
  вернёт в память.
...
(`torrcast/profile.py`): вес сегмента и кодеки, которые придётся перекодировать,
