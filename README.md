...
показа: «играю ... с 0:41:03 · выбрать другое: --menu». `--new` играет ту же сохранённую
...
занимает. Номер пункта (`--pick N`) и названная серия (`cast байки s1e1`) работают так
...
`scripts` (стенды и пробы - такие же законные вызывающие продукта, как команда `cast`),
